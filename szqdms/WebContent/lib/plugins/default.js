...
function doFormSubmit($f, sUrl, eventUrlId, curPage, param, tabId, dataType) {
	try {
		...
		} else if (!window.event.srcElement.tagName) $(document).trigger("ajaxStart");
		if (!param) sParam = {
			"data": ""
		};
		else sParam = {
...
function domToStr(xmlDom) {
	try {
		if (bOs == 1) {
			var s = xmlDom.xml;
			if (s == "" || s == undefined || s == "undefined" || s == null) s = new XMLSerializer().serializeToString(xmlDom);
			return s
...
function tabListShowResult(responseText, tabId, dType) {
	...
	case "xml":
		...
		if (nrecordsperpage <= 150) {
			...
			if ($("#" + tabId).attr("limitH") == "true") tabList.parent().height(lh);
			tabList.parent().css("overflow-y", "auto");
			$("th", tabList).each(function() {
				var $th = $(this);
				if ($th.hasClass("asc") || $th.hasClass("desc")) $th.click(function(event) {
					event.preventDefault();
					var ordertype = $th.attr("ordertype") ? $th.attr("ordertype") : "local";
					if (ordertype == 'local') {
						tabList.doSort($(this))
					}
				})
...
function doTabListCheck(obj) {
	var type = obj.type;
	var $tab = null;
	if ($("#" + $(obj).attr("tableid") + "_content").size() > 0) $tab = $("#" + $(obj).attr("tableid") + "_content");
	else $tab = $("#" + $(obj).attr("tableid"));
	if (type && type == 'checkbox') {
		...
		obj.checked = true;
		$("tr>td:first-child input[type='radio']", $tab).each(function() {
			var check = $(this);
			check.attr("checked", false)
		})
	}
