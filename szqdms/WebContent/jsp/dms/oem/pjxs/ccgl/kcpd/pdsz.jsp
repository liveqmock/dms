...
$(function(){
	...
	$("#btn-xz").bind("click",function(event){
		...
		$.pdialog.open(webApps + "/jsp/dms/oem/pjxs/ccgl/kcpd/pdszxz.jsp?action=1", "pdsz", "盘点设置新增", options,true);
	});
});
function doUpdate(){
	var options = {max:true,mask:true,mixable:true,minable:true,resizable:true,drawable:true};
	$.pdialog.open(webApps + "/jsp/dms/oem/pjxs/ccgl/kcpd/pdszxz.jsp?action=2", "ckdmx", "盘点设置编辑", options,true);
}
			...
			<div class="searchBar" align="left" >
				<table class="searchContent" id="tab-ckd">
					...
					<tr>
					    ...
					    <td>
					    	<select type="text" id="in-ddlx" name="in-ddlx" kind="dic" src="E#1=已保存:2=盘点中:3=已盘点" datasource="DDLX" datatype="1,is_null,30" operation="=">
			...
			<table style="display:none;width:100%;" id="tab-ddlb" name="tablist" ref="page_ddlb" refQuery="fm-pjcx" >
					<thead>
						<tr>
							...
							<th colwidth="150" type="link" title="[查看]|[修改]|[删除]"  action="doUpate|doSubmit|doDelete" >操作</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							...
							<td><div>已保存</div></td>
							<td><div>张三</div></td>
							<td><a href="#" onclick="doUpdate()" class="op">[编辑]</a><a href="#" onclick="doStart()" class="op">[盘点开始]</a><a href="#" onclick="doOver()" class="op">[盘点结束]</a></td>
						</tr>
						<tr>
							...
							<td><div>盘点中</div></td>
							<td><div>张三</div></td>
							<td><a href="#" onclick="doOver()" class="op">[盘点结束]</a></td>
						</tr>
						<tr>
							<td class="rownums"><div>3</div></td>
							<td style="display:none;"><div><input type="radio" name="in-xh" /></div></td>
							<td><div><a style="color:red;" onclick="toDetail('PDBH2014050901');">PDBH2014050501</a></div></td>
							<td><div>本部库</div></td>
							<td><div>部分</div></td>
							<td><div>2014-05-08</div></td>
							<td><div>已盘点</div></td>
