...
function toViewDetail(pdbh){
	var w = document.documentElement.clientWidth-100;
	var options = {max:false,width:w,height:500,mask:true,mixable:true,minable:true,resizable:true,drawable:true};
	$.pdialog.open(webApps + "/jsp/dms/oem/pjxs/ccgl/kcpd/pdjgfcmx.jsp", "fldmx", "盘点结果明细", options,true);
}
function toSubmit(){
			...
			<table style="display:none;width:100%;" id="tab-ddlb" name="tablist" ref="page_ddlb" refQuery="fm-pjcx" >
					...
					<tbody>
						<tr>
							...
							<td><div>张三</div></td>
							<td><div>财务已确认</div></td>
							<td><a href="#" onclick="toViewDetail()" class="op">[查看]</a><a href="#" onclick="toSubmit()" class="op">[封存]</a></td>
						</tr>
						<tr>
							...
							<td><div>张三</div></td>
							<td><div>已封存</div></td>
							<td><a href="#" onclick="toViewDetail()" class="op">[查看]</a></td>
						</tr>
					</tbody>
			</table>
		</div>
