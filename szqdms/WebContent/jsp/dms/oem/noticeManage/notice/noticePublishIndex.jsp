...
var deleteUrl = "<%=request.getContextPath()%>/service/noticeManage/NoticeManageAction/noticePublishdelete.ajax";
...
$(function(){
	...
	$("#btn-add").bind("click",function(event){
		...
		$.pdialog.open(webApps+"/jsp/dms/oem/noticeManage/notice/noticePublishEdit.jsp?action=1", "noticeManage", "通知通告内容新增", options,true); 
	});
});
function doDelete(rowobj)
{
	$row = $(rowobj);
	var url = deleteUrl + "?bulletinId="+$(rowobj).attr("BULLETIN_ID")+"";
	sendPost(url,"doDelete","",deleteCallBack,"true");
}
function doPublish(rowobj)
{
	$row = $(rowobj);
	var url = publishUrl + "?bulletinId="+$(rowobj).attr("BULLETIN_ID")+"";
...
function doUpdate(rowobj)
{
	...
	$.pdialog.open(webApps+"/jsp/dms/oem/noticeManage/notice/noticePublishEdit.jsp?action=2", "noticeManage", "通知通告内容修改", options,true);
}
...
<div id="layout" style="width:100%;">
		...
		<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath()%>/images/default/nav.gif" />&nbsp;当前位置：通知通告管理&gt;通知通告管理&gt;通告内容维护</h4>
		<div class="page">
			...
			<table style="display:none;width:100%;" layoutH="250" id="noticeManagelb" name="noticeManage" ref="noticeManage" refQuery="noticeManageTable" >
					<thead>
						<tr>
							...
							<th colwidth="145" type="link"  title="[修改]|[发布]|[删除]" action="doUpdate|doPublish|doDelete">操作</th>
