...
var searchUrl = "<%=request.getContextPath()%>/service/oldpartMng/OldPartCheckStorageAction/oldPartSearch.ajax";
...
$(function(){
	$("#search").bind("click",function(){
		...
		var sCondition = {};
		var url =diaAction+"/oldPartSearch.ajax?&claimNo="+claimNo+"&dealerCode="+dealerCode;
	...
	$("#export").bind("click",function(){
		var noptions = {max:false,mask:true,width:600,height:300,mixable:true,minable:true,resizable:true,drawable:true};
		$.pdialog.open(webApps + "/jsp/dms/oem/service/oldpart/except.jsp", "except", "旧件导出", noptions,false);
	});
	//导出 
	$("#download").bind("click",function(){
		...
		var claimNo=$("#claimNo").val();
		var dealerCode=$("#dealeOrgCode").val();
		$("#exportFm").attr("action","<%=request.getContextPath()%>/service/oldpartMng/OldPartCheckStorageAction/storageInDownLoad.do?&claimNo="+claimNo+"&dealerCode="+dealerCode);
		$("#exportFm").submit();
	});
	
	//导入
	$("#dia-imp").bind("click",function(){
        importXls("SE_BU_OLDPART_STORAGE_TMP","",15,3,"/jsp/dms/oem/service/oldpart/importSuccess2.jsp");
	});
});

function doUpdate(rowobj){
	$("td input[type=radio]",$(rowobj)).attr("checked",true);
	$.pdialog.open(webApps + "/jsp/dms/oem/service/oldpart/oldPartStorageInDetail.jsp", "oldPartStorageIn", "旧件入库", options,true);
}
...
<div id="layout" style="width:100%;">
		...
		<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath()%>/images/default/nav.gif" />&nbsp;当前位置：旧件管理&gt;旧件管理&gt;旧件入库</h4>
		<div class="page">
				...
				<div class="searchBar" align="left">
					...
					<div class="subBar">
						<ul>
							...
							<li><div class="buttonActive"><div class="buttonContent"><button type="button" id="export">审核导出</button></div></div></li>
		...
		<div class="panelBar">
			<ul class="toolBar">
				...
				<li><a class="icon" href="###" id="download" ><span>入库导出</span></a></li>
				<li class="line">line</li>
				<li><a class="add" href="javascript:void(0);" id="dia-imp" title="确定要导入吗?"><span>导入入库数据</span></a></li>
			</ul>
		</div>
