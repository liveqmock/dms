...
$(function(){
	...
	$("#export").bind("click",function(){
		...
		$("#exportFm").attr("action","<%=request.getContextPath()%>/service/claimmng/WorkOrderMngAction/oemDownload.do");
		$("#exportFm").submit();
	});
});
//工单号链接
			...
			<div class="searchBar" align="left" >
				<table class="searchContent" id="tab-workOrderSearch">
					...
					<tr>
						...
						<td><input type="text" id="orgCode" name="orgCode" datasource="T.ORG_ID" datatype="1,is_null,10000" operation="in" value="" hasBtn="true"  code="" callFunction="showOrgTree('orgCode',2)" readonly="readonly"/></td>
					...
					<tr>
						<td><label>派单日期：</label></td>
						<td ><input type="text" group="in-pdrqks,in-pdrqjs"  id="in-pdrqks"  name="in-pdrqks" style="width:75px;" datasource="T.JOBORDER_TIME" datatype="1,is_null,10" onclick="WdatePicker()"  kind="date" operation=">="/>
				    		<span style="float:left;margin-left:-50px;margin-top:5px;">至</span>
							<input type="text" group="in-pdrqks,in-pdrqjs"  id="in-pdrqjs"  name="in-pdrqjs" style="width:75px;margin-left:-30px;"  datasource="T.JOBORDER_TIME" datatype="1,is_null,10" kind="date" operation="<=" onclick="WdatePicker()" />
				   		 </td>
				   		 <td><label>派工日期：</label></td>
						 <td ><input type="text" group="in-pgrqks,in-pgrqjs"  id="in-pgrqks"  name="in-pgrqks" style="width:75px;" datasource="T.DISPATCH_TIME" datatype="1,is_null,10" onclick="WdatePicker()"  kind="date" operation=">="/>
				    		<span style="float:left;margin-left:-50px;margin-top:5px;">至</span>
							<input type="text" group="in-pgrqjs,in-pgrqjs"  id="in-pgrqjs"  name="in-pgrqjs" style="width:75px;margin-left:-30px;"  datasource="T.DISPATCH_TIME" datatype="1,is_null,10" kind="date" operation="<=" onclick="WdatePicker()" />
				   		 </td>
			...
			<table style="display:none;width:100%;" id="tab-workOrderList" name="tablist" ref="page_wrokOrderList" refQuery="fm-searchWorkOrder" >
					<thead>
						<tr>
							...
							<th fieldname="JOBORDER_TIME"colwidth="120" ordertype='local' class="desc">派单时间</th>
							<th fieldname="DISPATCH_TIME"colwidth="120" ordertype='local' class="desc">派工时间</th>
							...
							<th fieldname="COMPLETE_DATE" colwidth="120" ordertype='local' class="desc">完工时间</th>
							<th fieldname="WORK_TYPE" colwidth="80">派工类型</th>
							<th fieldname="IF_OUT" colwidth="80">是否有外出</th>
						</tr>
					</thead>
			</table>
		</div>
