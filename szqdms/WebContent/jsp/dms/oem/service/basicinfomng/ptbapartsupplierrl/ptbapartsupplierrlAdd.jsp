			...
			<div align="left">
				...
				<table class="editTable" id="dia-tab-pjxx">										
					<tr>
					    ...
					    <td><input type="text" id="dia-supplier_code"  name="dia-supplier_code" kind="dic"  datasource="SUPPLIER_CODE"  src="T#PT_BA_SUPPLIER:SUPPLIER_CODE:SUPPLIER_NAME{SUPPLIER_ID,SUPPLIER_NAME}:1=1 AND SE_IDENTIFY=<%=DicConstant.YXBS_01 %> ORDER BY SUPPLIER_ID" datatype="0,is_null,300"/>		
...
<!--					</tr>		-->
					...
					<tr>
					    ...
					    <td>
					    	<input type="text" id="dia-paravalue1" name="dia-paravalue1" kind="dic" src="T#USER_PARA_CONFIGURE:PARAVALUE2:PARAVALUE1{PARAVALUE2,PARAVALUE1}:1=1 AND APPTYPE='3001' AND STATUS='100201'"  datasource="PARAVALUE1" datatype="1,is_null,100" operation="=" readonly="true" />
					    	<input type="hidden" id="dia-apply_cycle" name="dia-apply_cycle" datasource="APPLY_CYCLE" />
							<input type="hidden" id="dia-paravalue1" name="dia-paravalue1" datasource="PARAVALUE1" />
					    </td> -->
					     <td><label>服务标识：</label></td>
			   		 	<td>
				    	<select type="text" class="combox" id="dia-se_identify" name="dia-se_identify" kind="dic" src="YXBS" datasource="SE_IDENTIFY" datatype="0,is_null,100" >	
...
//修改操作
	if(diaAction != "1")
	{
		// 此处Bug：table选择列为checbox，可能选择多个，且主页面不能根据radio定位选择行。
 		// var selectedRows = $("#tab-pjlb").getSelectedRows();
		// 此处改为编辑时，主页面保留行对象，div根据临时参数获取行信息，使用完毕后在将临时对象重置为空数组
 		var selectedRows = selectRowObj;
...
/*TD中放大镜按钮响应事件  */
	function openPurchase()
	{
		...
		var url = webApps +"/jsp/dms/oem/service/basicinfomng/ptbapartsupplierrl/ptbapartsupplierrlSel.jsp";	
