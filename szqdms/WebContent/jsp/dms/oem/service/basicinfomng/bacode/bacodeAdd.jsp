...
<div id="dia-layout">
	...
	<div class="pageContent" style="" >
		<form method="post" id="fm-bacodeInfo" class="editForm" >
			<!-- 隐藏域 -->
			...
			<div align="left">
			<fieldset>
			<table class="editTable" id="tab-bacodeInfo">
				<tr>
					<td><label>代码类别：</label></td>
					<td>
		        		<select id="dia-codeType"  name="dia-codeType" datasource="CODE_TYPE" kind="dic" src="DMLB" datatype="0,is_null,6" >
				...
				<tr>
					...
					<td><input type="text" id="dia-name" name="dia-name" datasource="NAME" datatype="0,is_null,30"  /></td>
				</tr>		   
                <tr>
...
var diaSaveAction = "<%=request.getContextPath()%>/service/basicinfomng/BaCodeMngAction";
...
$(function(){
	...
	$("#btn-save").bind("click", function(event){
		...
		var sCondition = {};
		//将需要提交的内容拼接成json
		sCondition = $("#fm-bacodeInfo").combined(1) || {};
		if(diaAction == 1)	//插入动作
		{
	...
	if(diaAction != "1")
	{
		...
		var selectedRows = $("#tab-bacodelist").getSelectedRows();
		//setEditValue：设置输入框赋值方法
		//"fm-userInfo"：要赋值区域的id
		//selectedRows[0].attr("rowdata"):行对象属性rowdata中保存了xml格式的行数据
		setEditValue("fm-bacodeInfo",selectedRows[0].attr("rowdata"));
		//设置置灰值
...
{
	//设置默认值有两种方式
	//1.如代码类别，在select里写<option value="-1" selected>--</option>，在函数中声明如下
	//2.如有效标示，在select里写<option value="<=DicConstant.YXBS_01 %>"有效</option>，函数中不用声明
	
	//代码类别
	$("#dia-codeType").val("<%=DicConstant.DMLB_02%>");
	$("#dia-codeType").attr("code","<%=DicConstant.DMLB_02%>");
	$("#dia-codeType").find("option").val("<%=DicConstant.DMLB_02%>");
	$("#dia-codeType").find("option").text("故障类别");
}
...
function diaInsertCallBack(res)
{
	...
	{
		//第二个参数0表示插入到第几行
		$("#tab-bacodelist").insertResult(res,0);
	}catch(e)
	{
		alertMsg.error(e);
		return false;
...
function diaUpdateCallBack(res)
{
	try
	{
		var selectedIndex = $("#tab-bacodelist").getSelectedIndex();
		$("#tab-bacodelist").updateResult(res,selectedIndex);
		//关闭当前窗口
