...
function showAnNiu(obj){
	var $tr = $(obj).parent();
	var HDZT = $tr.find("td").eq(9).attr("code");
	if(HDZT==<%=DicConstant.HDZT_01%>||HDZT==<%=DicConstant.HDZT_03%>){
	obj.html("<A class=op title=[发布] onclick='doPublish(this.parentElement.parentElement.parentElement)' href='javascript:void(0);'>[发布]</A>");
	}
	if(HDZT==<%=DicConstant.HDZT_02%>){
	obj.html("<A class=op title=[取消] onclick='doCancel(this.parentElement.parentElement.parentElement)' href='javascript:void(0);'>[取消]</A><A class=op title=[关闭] onclick=doClose(this.parentElement.parentElement.parentElement) href='javascript:void(0);'>[关闭]</A>");
	}
}
//取消
function doCancel(rowobj){
	...
	$row=$(rowobj);
	var activityId=$row.attr("ACTIVITY_ID");
	var addUrl = diaSaveAction + "/doCancel.ajax?activityId="+activityId;
	sendPost(addUrl,"","",updateHdztCallBack,"true");
}
//关闭
function doClose(rowobj){
	...
	$row=$(rowobj);
	var activityId=$row.attr("ACTIVITY_ID");
	var addUrl = diaSaveAction + "/doClose.ajax?activityId="+activityId;
	sendPost(addUrl,"","",updateHdztCallBack,"true");
}
//发布
function doPublish(rowobj){
	$("td input[type=radio]",$(rowobj)).attr("checked",true);
	$row=$(rowobj);
	var activityId=$row.attr("ACTIVITY_ID");
	var addUrl = diaSaveAction + "/doPublish.ajax?activityId="+activityId;
	sendPost(addUrl,"","",updateHdztCallBack,"true");
}
//新增促销配件回调
function updateHdztCallBack(){
	var $f = $("#serviceActivityScopeTableform");//获取页面提交请求的form对象
	var sCondition = {};//定义json条件对象
	//combined()：实现将页面条件按规则拼接成json
	sCondition = $f.combined() || {};
...
<div id="layout" style="width:100%;">
		...
		<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath()%>/images/default/nav.gif" />&nbsp;当前位置：服务活动管理&gt;服务活动发布</h4>
		<div class="page">
				...
				<table style="display:none;width:100%;" id="tab-serviceActivity" name="tablist" ref="page_serviceActivity" refQuery="tab-serviceActivity" >
						<thead>
							<tr>
								...
								<th colwidth="100" type="link" refer="showAnNiu" title="[发布]|[取消]|[关闭]" action="doPublish|doUpdate|doDelete">操作</th>
							</tr>
						</thead>
						<tbody>
						</tbody>
