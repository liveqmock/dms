...
$(function(){
	...
	$("#exp").bind("click",function(){
		...
		$("#exportFm").attr("action","<%=request.getContextPath()%>/service/financeMng/StatementAdjustAction/downloadPolicyTemplet.do");
		$("#exportFm").submit();
	});
	//导入
	$("#imp").bind("click",function(){
        ...
        //最后一个参数表示 导入成功后显示页
        importXls("SE_BU_CLAIM_SETTLE_TMP_POLICY",'', 8, 3,"/jsp/dms/oem/service/finance/importPolicyFeetsSuccess.jsp");
	});
	//生产开票通知单
...
function doUpdate(rowobj){
	...
	$.pdialog.open(webApps+"/jsp/dms/oem/service/finance/policyFeesDetail.jsp", "statementAdjust", "结算单调整明细", options,true);
}
//导入回调方法
function impCall(){
	var $f = $("#settleform");
...
<div id="layout" style="width:100%;">
		...
		<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath()%>/images/default/nav.gif" />&nbsp;当前位置：财务管理&gt;结算管理&gt;政策支持费用调整</h4>
		<div class="page">
				...
				<table style="display:none;width:100%;" id="settleList" name="settleList"  ref="settle" refQuery="settleTable">
					<thead>
						<tr>
							...
							<th colwidth="80" fieldname="POLICY_ADJUST_USER" >调整人</th>
							<th colwidth="80" fieldname="POLICY_ADJUST_TIME" >调整时间</th>
