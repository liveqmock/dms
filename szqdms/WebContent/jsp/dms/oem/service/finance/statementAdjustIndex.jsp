...
//查询按钮响应方法
var searchUrl = "<%=request.getContextPath()%>/service/financeMng/StatementAdjustAction";
var invoiceUrl = "<%=request.getContextPath()%>/service/financeMng/StatementAdjustAction/settleUpdate.ajax";
$(function(){
	$("#search").bind("click",function(event){
		$("#ifmakeamount").attr("action","show");
		var $f = $("#settleform");
		var sCondition = {};
    	sCondition = $f.combined() || {};
    	var ifmake=$("#ifmakeamount").val();
    	var url =searchUrl+"/settleSearch.ajax?&ifmake="+ifmake;
		doFormSubmit($f,url,"search",1,sCondition,"settleList");
	});
	//下载
	$("#exp").bind("click",function(){
		...
		$("#exportFm").attr("action","<%=request.getContextPath()%>/service/financeMng/StatementAdjustAction/download.do?&ifmake="+ifmake);
		$("#exportFm").submit();
	});
	//导入
	$("#imp").bind("click",function(){
        ...
        //最后一个参数表示 导入成功后显示页
        // 主功能屏蔽两列，由原来的15变为13
        importXls("SE_BU_CLAIM_SETTLE_TMP",'',16,3,"/jsp/dms/oem/service/finance/importSuccess.jsp");
	});
	//生产开票通知单
...
function doUpdate(rowobj){
	...
	$.pdialog.open(webApps+"/jsp/dms/oem/service/finance/statementAdjustDetail.jsp", "statementAdjust", "结算单调整明细", options,true);
}
//导入回调方法
function impCall(){
	var $f = $("#settleform");
...
<div id="layout" style="width:100%;">
		...
		<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath()%>/images/default/nav.gif" />&nbsp;当前位置：财务管理&gt;结算管理&gt;结算单调整</h4>
		<div class="page">
				...
				<div class="searchBar" align="left">
					<table class="searchContent" id="settleTable">
						<tr>
							...
							<td>
								<select type="text" id="ifmakeamount" name="ifmakeamount" class="combox" kind="dic" src="SF"  datatype="1,is_null,6"> 
		...
		<div class="pageContent">
			<div id="settle" >
				<table style="display:none;width:100%;overflow:auto" id="settleList" name="settleList"  ref="settle" refQuery="settleTable">
					<thead>
						<tr>
							...
							<th colwidth="80" fieldname="ADJUST_USER" >调整人</th>
							<th colwidth="80" fieldname="ADJUST_TIME" >调整时间</th>
							...
							<th align="right" colwidth="60" fieldname="PART_MATERIAL_COSTS" refer="amountFormat">配件三包材料费</th>
							<th align="right" colwidth="60" fieldname="MATERIALCOST_REDUCE" refer="amountFormat">材料费冲减</th>
							<th align="right" colwidth="60" fieldname="MANUALLY_COST" refer="amountFormat">手工帐费用</th>
							<!-- <th align="right" colwidth="60" fieldname="PART_MATERIAL_COSTS" refer="amountFormat">配件索赔材料费</th> -->
