...
$(function()
{
	...
	$("#dia_seOthers").bind("blur",function(){
		if (is_all_digit_2($("#dia_seOthers")) === true) {
			doSeSummary();
		}else{
	...
	$("#dia_save").bind("click",function(){
		...
		if (submitForm($form) === true) {
			...
			var addUrl = diaSaveAction + "/settleAdjustSave.ajax?flag=3";
...
function doSeSummary(){
	...
	if ($("#dia_seOthers").val()=='') {
		...
		seOthers = parseFloat($("#dia_seOthers").val());
		seOthers = isNaN(seOthers) ? 0 : seOthers;
	}
	//汇总
			...
			<legend align="right"><a id="typeH" onclick="onTitleClick('dia_settleDetailSeTable')">&nbsp;服务费用信息&gt;&gt;</a></legend>
				<table class="editTable" id="dia_settleDetailSeTable">
					...
					<tr>
						...
						<td><input type="text" id="dia_seOthers" name="dia_seOthers" datasource="OTHERS"  datatype="0,is_null,30"  value="" /></td>
					...
					<tr>
					    ...
					    <td colspan="2"><textarea class="" rows="2" id="dia_seRemarks" name="dia_seRemarks" datasource="REMARKS" style="width:100%" datatype="1,is_null,500" readonly="readonly"></textarea></td>
					</tr>
				</table>
		  	</fieldset>
			</div>
