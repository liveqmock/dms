...
$(function()
{
	$("#btn-search").bind("click", function(event){
		var searchUrl = "<%=request.getContextPath()%>/part/purchaseMng/purchaseContract/ContractOperationManageAction/signContractSearch.ajax";
...
{
	$row = $(rowobj);
	var reportUrl = "<%=request.getContextPath()%>/part/purchaseMng/purchaseContract/ContractOperationManageAction/contractSign.ajax";
	var url = reportUrl + "?CONTRACT_ID="+$(rowobj).attr("CONTRACT_ID");
	sendPost(url,"sign","",signCallBack,"true");
}
			...
			<table style="display:none;width:100%;" id="tab-contract" name="tablist" ref="page_contract" refQuery="fm-htcx" >
					<thead>
						<tr>
							...
							<th colwidth="50" type="link" title="[签订]"  action="doSign" >操作</th>
						</tr>
					</thead>
			</table>
		</div>
