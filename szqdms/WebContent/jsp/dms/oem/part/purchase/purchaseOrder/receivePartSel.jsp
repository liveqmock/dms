    ...
    <div class="page">
                ...
                <div class="searchBar" align="left">
                    <table class="searchContent" id="tab-searchPart">
                        ...
                        <tr>
                            ...
                            <td><input type="text" id="dia-PART_CODE" name="dia-PART_CODE" datasource="T.PART_CODE"
                                       datatype="1,is_null,30" operation="like"/></td>
                            <td><label>配件名称：</label></td>
                            <td><input type="text" id="dia-PART_NAME" name="dia-PART_NAME" datasource="T.PART_NAME"
                                       datatype="1,is_null,30" operation="like"/></td>
                        </tr>
                    </table>
                    <div class="subBar">
        ...
        <div class="pageContent">
                ...
                <table style="display:none;width:100%;" id="tab-partList" multivals="div-selectedPart" name="tablist" ref="div-partList"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th fieldname="PCH_COUNT" refer="createInputBox">领用数量</th>
                            <th fieldname="PCH_PRICE">采购价格(元)</th>
                            <th fieldname="PCH_AMOUNT">金额(元)</th>
                            <th fieldname="PB_NO" refer="createInput">计配号</th>
                            <th fieldname="REMARK" refer="createRemarkInput">备注</th>
                           </tr>
                    </thead>
                </table>
            </div>
            ...
            <form id="fm-partInfo" method="post">
                <input type="hidden" id="recId" name="recId" datasource="RECID"/>
                ...
                <input type="hidden" id="remarks" name="remarks" datasource="RMKS"/>
            </form>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#btn-searchPart").bind("click", function (event) {
            //查询配件URL
            var searchPartUrl = url+"/receiveRearchPart.ajax?SPLIT_ID="+$("#dia-SPLIT_ID").val()+"&REC_ID="+$("#dia-REC_ID").val();
        ...
        $('#btn-confirmPart').bind('click',function(){
            //添加促销配件URL
            var insertPromPartUrl = url+"/insertReceiveOrderPart.ajax";
            var partIds = $('#val0').val();
            if(!partIds){
                alertMsg.warn('请选择配件!')
            }else{
                $('#recId').val($('#dia-REC_ID').val());//促销活动ID
                $('#partIds').val(partIds);
    ...
    {
        return '<input type="text" name="PCH_COUNT" value="'+obj.text()+'" onblur="doMyInputBlur(this)"/>';
    }
    
    function createInput(obj)
    {
        return '<input type="text" name="PBNO" onblur="doMyInputCheck(this)"/ >';
    }
    function createRemarkInput(obj)
    {
        return '<input type="text" name="REMARK" onblur="doMyInputRemark(this)"/ >';
    }
    ...
    function doCheckbox(checkbox) {
        ...
        var p = $tr.find("td").eq(9).find("input:first").val();
    ...
    function customOtherValue($row,checkVal)
    {
        ...
        var $inputObj2 = $row.find("td").eq(9);
        var $inputObj3 = $row.find("td").eq(10);
        var val;
        var val1;
        var val2;
        if($("#val3") && $("#val3").val())
        ...
        if(val2)
        {
            $inputObj2.html("<input type='text' name='PBNO' onblur='doMyInputCheck(this)' value='"+val2+"'/ >");
        }
        ...
        if(val3)
        {
            $inputObj2.html("<input type='text' name='REMARK' onblur='doMyInputRemark(this)' value='"+val3+"'/ >");
        }
    }
</script>