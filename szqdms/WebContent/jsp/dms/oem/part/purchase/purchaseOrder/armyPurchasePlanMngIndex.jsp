...
var mngUrl = "<%=request.getContextPath()%>/part/purchaseMng/purchaseOrder/ArmyPurchasePlanMngAction";
$(function()
{
	...
	$("#page_order").height(document.documentElement.clientHeight-100);
	...
	$("#btn-search").bind("click", function(event){
		var type = $("#PURCHASE_TYPE").val();
		var searchUrl = mngUrl+"/partReSearch.ajax";
		var $f = $("#fm-searchPchOrder");
		var sCondition = {};
    	sCondition = $f.combined() || {};
		sendPost(searchUrl, "btn-search", sCondition, doSearchCallBack, "");
...
/*             	var partId=$(this).find("td").eq(0).find("input:first").val();
	...
	$('#btn-saveTmp').bind('click',function(){
        ...
        var type = $("#PURCHASE_TYPE").val();
        var shippingUrl = mngUrl+"/savePurchaseOrderTmp.ajax?account="+userAcc;
        var partIds="";
        var supplierIds="";
        var myCounts="";
        var remarks="";
        ...
        for(var i=0;i<rows.length;i++){
            ...
            if(ifSuply==<%=DicConstant.SF_01%>)
            {
                ...
                var s = "<td style='display:none;'><input type='checkbox' checked='true' name='keyIds' onclick='doCheckbox(this);' value='"+getNodeText(rows[i].getElementsByTagName("KEY_ID").item(0))+"'";
                    ...
                    /* s += " applyUser='"+getNodeText(rows[i].getElementsByTagName("USER_ACCOUNT").item(0))+"' "; */
                ...
                $row.append($td6);
/*                 var $td7 = $("<td>"+getAttribValue(objXML, "USER_ACCOUNT","sv", 0)+"</td>");
                $row.append($td7);  */
/*                 var applyUser = getAttribValue(objXML, "USER_ACCOUNT","", 0);
                var applyUserName = getAttribValue(objXML, "USER_ACCOUNT","sv", 0);
               	var $td7= $("<td><input type='hidden' name='applyUser' value='"+applyUser+"'/>"+applyUserName+"</td>"); */
               	$row.append($td7);
...
/*                             s += " applyUser='"+getNodeText(rows[i].getElementsByTagName("USER_ACCOUNT").item(0))+"' "; */
                    ...
                    $row.append($td6);
/*                     var $td7 = $("<td rowspan='"+len+"'>"+getAttribValue(objXML, "USER_ACCOUNT","sv", 0)+"</td>");
                    $row.append($td7); */
...
<div id="layout" style="width:100%;">
	...
	<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath() %>/images/default/nav.gif" />&nbsp;当前位置： 采购管理  &gt; 采购订单管理   &gt; 军品采购计划制定</h4>
	<div class="page">
			...
			<div class="searchBar" align="left" >
				<table class="searchContent" id="tab-orderSearch">
					...
					<tr>
						...
						<td>
					    	<input type="text" id="PERSON_NAME" name="PERSON_NAME" action="show" datasource="T0.USER_ACCOUNT" kind="dic" src="" datatype="1,is_null,300" readonly="true"/>
						</td>
					</tr>
					<tr>
					    <td style="display:none"><select type="text" class="combox" id="PURCHASE_TYPE" name="PURCHASE_TYPE" kind="dic" src="CGDDLX"  filtercode="<%=DicConstant.CGDDLX_04%>" datatype="0,is_null,6" readonly="readonly">
							    	<option value="<%=DicConstant.CGDDLX_04%>" selected>缺件</option>
					    </select></td>
	...
	<div class="pageContent">
      ...
      <form id="fm-partInfo">
        ...
        <input type="hidden" id="sApplyUsers" datasource="APPLY_USERS"/>
   	  </form>
