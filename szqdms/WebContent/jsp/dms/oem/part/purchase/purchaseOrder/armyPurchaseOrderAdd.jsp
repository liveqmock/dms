            ...
            <div class="page">
                ...
                <div class="formBar">
                    <ul>
                        ...
                        <li id="dia-export" ><div class="button"><div class="buttonContent"><button type="button" id="dia-export" >明细导出</button></div></div></li>
...
/* 取父页面传过的参数确定新增或者修改 */
    ...
    var diaUrl = "<%=request.getContextPath()%>/part/purchaseMng/purchaseOrder/ArmyPurchaseOrderMngAction";
    var flag = true;
    $(function () {
//        $.purchaseOrderMng.init();
    ...
    $("button[name='btn-next']").bind("click",function(event){
        ...
        $("#btn-save").bind("click", function(event){
			...
			if(diaAction == 1)
			{
				var addUrl = diaUrl + "/purchaseOrderInsert.ajax";
        ...
        $("#btn-report").bind("click", function(event){
    		...
    		sendPost(reportUrl,"report","",reportInCallBack,"true"); 
    	});
    	//关闭按钮响应
    	...
    	$('#btn-import').bind('click', function () {
            ...
            //最后一个参数表示 导入成功后显示页
            importXls("ARMY_PT_BU_PCH_ORDER_PART_TMP",$('#dia-PURCHASE_ID').val(),4,3,"/jsp/dms/oem/part/purchase/purchaseOrder/importSuccess.jsp");
        });
    	$("#dia-export").bind("click",function(){
    		$("#exportFm").attr("action","<%=request.getContextPath()%>/part/purchaseMng/purchaseOrder/ArmyPurchaseOrderMngAction/dtlDownload.do?PURCHASE_ID="+$('#dia-PURCHASE_ID').val());
    		$("#exportFm").submit();
    	});
		//修改页面赋值
        if (diaAction != "1") {
    ...
    function addParts() {
        var poptions = {max: false, width: 900, height: 500, mask: true, mixable: true, minable: true, resizable: false, drawable: true};
        $.pdialog.open(webApps + "/jsp/dms/oem/part/purchase/purchaseOrder/armyOrderPartSel.jsp", "partSelWin", "配件信息查询", poptions);
    }
    //采购订单配件删除
    var $row;
    ...
    var $row;
    function reportInCallBack(){
    	var supplierCode = $("#dia-SUPPLIER_CODE");
		if('SQGH001'==supplierCode){
			$.pdialog.closeCurrent();
			doSearchOrder(); 
		}else{
			doApproveOk1();
	...
	function openSupplier()
	{
		...
		var url = webApps +"/jsp/dms/oem/part/purchase/purchaseOrder/supplierSel.jsp?&TYPE=2&ACCOUNT="+$("#PERSON_NAME").attr("code");
