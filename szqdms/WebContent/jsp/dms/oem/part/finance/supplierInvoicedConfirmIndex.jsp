...
var diaAddOptions = {max:true,width:800,height:430,mask:true,mixable:true,minable:true,resizable:true,drawable:true};
var mngUrl = "<%=request.getContextPath()%>/part/financeMng/invoiceNotice/InvoiceConfirmMngAction";
$(function()
{
...
function doConfirm(rowobj)
{
	...
	$.pdialog.open(webApps+"/jsp/dms/oem/part/finance/supplierInvoiceConfirm.jsp?SUM_ID="+SUM_ID, "确认页面", "确认页面", diaAddOptions);
}
...
function afterDicItemClick(id,$row){
	...
	if(id == "SUPPLIER_CODE")
	{
		...
		$("#SUM_ID").val($("#"+id).attr("SUPPLIER_ID"));
		
	}
	return ret;
}
function doSearchSum(){
	var searchUrl = mngUrl+"/invioceSearch.ajax";
	var $f = $("#fm-searchInvioce");
	var sCondition = {};
	sCondition = $f.combined() || {};
...
<div id="layout" style="width:100%;">
	...
	<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath() %>/images/default/nav.gif" />&nbsp;当前位置： 财务管理  &gt; 供应商开票管理   &gt; 开票确认</h4>
	<div class="page" >
			...
			<table style="display:none;width:100%;" id="tab-invoice_info" name="tablist" ref="page_invoice" refQuery="fm-searchInvioce" >
					<thead>
						<tr>
							...
							<th fieldname="HAS_INVOICE_AMOUNT" align="right" refer="amountFormatStr">开票金额汇总</th>
							<th colwidth="105" type="link" title="[开票确认]"  action="doConfirm" >操作</th>
						</tr>
					</thead>
					<tbody>
                    </tbody>
