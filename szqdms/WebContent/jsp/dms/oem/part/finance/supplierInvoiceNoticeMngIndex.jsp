...
var mngUrl = "<%=request.getContextPath()%>/part/financeMng/invoiceNotice/InvoiceNoticeMngAction";
$(function()
{
	...
	$("#btn-notice").bind("click",function(event){
		...
		{
			//跳转到保存方法
			var noticeUrl =mngUrl+"/noticeSupplier.ajax?SUM_ID="+id;
	...
	$("#btn-download").bind("click",function(){
		var id = $("#SUM_ID").val();
		if(id){
			$("#fm-searchInvioce").attr("action",mngUrl+"/download.do?SUPPLIER_ID="+id);
		}else{
			$("#fm-searchInvioce").attr("action",mngUrl+"/download.do");
		}
		
		$("#fm-searchInvioce").submit();
	});
...
function doDetail(rowobj)
{
	...
	$.pdialog.open(webApps+"/jsp/dms/oem/part/finance/supplierInvoiceDetail.jsp?SUM_ID="+SUM_ID, "明细页面", "明细页面", diaAddOptions);
}
var row;
			...
			<div class="searchBar" align="left" >
				...
				<div class="subBar">
					<ul>
						...
						<li><div class="button"><div class="buttonContent"><button type="button" id="btn-notice" >通知开票</button></div></div></li>
					</ul>
				</div>
			</div>
		</form>
	...
	<div class="pageContent">
		<div id="page_invoice" >
			<table style="display:none;width:100%;" id="tab-invoice_info" name="tablist" ref="page_invoice" refQuery="fm-searchInvioce" >
					<thead>
						<tr>
							<th type="multi" name="XH" unique="SUM_ID" style=""></th>
