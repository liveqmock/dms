...
$(function(){
	...
	$("#btn-xz").bind("click",function(event){
		...
		$.pdialog.open(webApps + "/jsp/dms/oem/part/basicInfo/ptbapartsupplierrl/ptbapartsupplierrlAdd.jsp?action=1", "pjzpxz", "配件供货关系信息新增", options);
							      
	});	
	$('#btn-download').bind('click', function () {
        var url = encodeURI("<%=request.getContextPath()%>/jsp/dms/common/download.jsp?filename=partSupplierRlImp.xls");
        window.location.href = url;
    });
	$('#btn-import').bind('click', function () {
        importXls("PT_BA_PART_SUPPLIER_RL_TMP",<%=DicConstant.PJZT_01%>,5,3,"/jsp/dms/oem/part/basicInfo/ptbapartsupplierrl/ppRlImportSuccess.jsp");
    });
    //供货关系模版导出按钮响应
	...
	$("#btn-export").click(function(){
		...
		$("#exportFm").attr("action","<%=request.getContextPath()%>/part/basicInfoMng/PtBaPartSupplierRlAction/exportExcel.do");
		$("#exportFm").submit();
	})
...
function doUpdate(rowobj)
{
	...
	$.pdialog.open(webApps+"/jsp/dms/oem/part/basicInfo/ptbapartsupplierrl/ptbapartsupplierrlAdd.jsp?action=2", "editPjzp", "修改配件供货关系信息", diaAddOptions);
}
...
<!--					    </td>-->
				...
				<div class="subBar">
					<ul>
						...
						<li><div class="button"><div class="buttonContent"><button type="button" id="btn-import" >导&nbsp;&nbsp;入</button></div></div></li>
					</ul>
				</div>
			</div>
		</form>
			...
			<table style="display:none;width:100%;" id="tab-pjlb" name="tablist" ref="page_yhlb" refQuery="fm-pjcx" >
					<thead>
						<tr>
							...
							<th fieldname="PART_CODE" colwidth="140">配件代码</th>							
							<th fieldname="PART_NAME" colwidth="140">配件名称</th>
							<th fieldname="SUPPLIER_CODE" colwidth="75">供应商代码</th>
...
<!--							<th fieldname="IF_STREAM" >是否需要流水号</th>-->
							<th fieldname="APPLY_CYCLE" colwidth="55" >供货周期</th>	
							<th fieldname="PART_IDENTIFY" colwidth="55">配件标识</th>
							<th fieldname="PART_STATUS" colwidth="55">配件状态</th>
							<th fieldname="UPDATE_USER" colwidth="55">修改人</th>	
							<th fieldname="UPDATE_TIME" colwidth="75">修改时间</th>	
							<th colwidth="70" type="link" title="[编辑]"  action="doUpdate" >操作</th>
						</tr>
					</thead>					
			</table>
		</div>
