...
<div id="dia-layout">
	...
	<div class="pageContent" style="" >
		<form method="post" id="dia-fm-pjdawh" class="editForm" >
			...
			<div align="left">
				<fieldset>
				<legend align="right"><a onclick="onTitleClick('dia-tab-pjxx')">&nbsp;服务索赔价格编辑&gt;&gt;</a></legend>
				<table class="editTable" id="dia-tab-pjxx">		
					<tr>
					    <td><label>配件代码：</label></td>
					    <td><input type="text" id="dia-part_code"  name="dia-part_code" datasource="PART_CODE" datatype="0,is_null,300" readonly="true"/>	
					    <input type="hidden" id="dia-part_id" name="dia-part_id" datasource="PART_ID" />				   			    
					    </td>
					    
					     <td><label>配件名称：</label></td>
					    <td><input type="text" id="dia-in-pjmc"  name="dia-in-pjmc" datasource="PART_NAME" datatype="0,is_null,300" readonly="true"/></td>
					</tr>
					<tr>						
					    <td><label>服务索赔价格：</label></td>
					    <td><input type="text" id="dia-se_clprice"  name="dia-se_clprice" datasource="SE_CLPRICE" datatype="0,is_digit_letter,30" value=""/></td>
					</tr>
				</table>
				</fieldset>	
			</div>
...
//修改操作
	if(diaAction != "1")
	{
		...
		if(!$("#dia-se_clprice").val()){
			 $('#dia-se_clprice').val("0.00");
		}
...
$(function(){
	...
	$("#btn-save").bind("click", function(event){
		...
		var se_clprice = $("#dia-se_clprice").val();	
		if (!pattern.test(se_clprice)) {
			alert("服务索赔价格必须符合财务数字，如12.65或12");
			return false;
		}
		var original_price=selectedRows[0].attr("SE_CLPRICE");
		var now_price=$('#dia-se_clprice').val();
		if(now_price==original_price){
       	alert("价格未做任何调整");
       }else{
       	var updateUrl = diaSaveAction + "/updateSeClPrice.ajax?original_price="+original_price+"&now_price="+now_price;
