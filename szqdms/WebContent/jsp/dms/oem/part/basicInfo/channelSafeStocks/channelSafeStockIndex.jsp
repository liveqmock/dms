...
<title>渠道安全库存管理</title>

<script src="<%=request.getContextPath() %>/lib/plugins/mine.filestore.js" type="text/javascript"></script>
<script src="<%=request.getContextPath() %>/lib/plugins/importXls.js" type="text/javascript"></script>
<script type="text/javascript">

var searchUrl = "<%=request.getContextPath()%>/part/basicInfoMng/ChannelSafeStockAction/search.ajax";
var doReSetUrl = "<%=request.getContextPath()%>/part/basicInfoMng/ChannelSafeStockAction/doReSet.ajax";
//定义弹出窗口样式
...
$(function()
{
	...
	$("#btn-search").bind("click", function(event){
		var $f = $("#fm-searchChannelSafeStock");//获取页面提交请求的form对象
		//判断上下限的指的合理性下限<上限
		...
		doFormSubmit($f,searchUrl,"btn-search",1,sCondition,"tab-channelSafeStockList");
	});
	
	//配件模版导出按钮响应
    $('#btn-expTpl').bind('click', function () {
        var url = encodeURI("<%=request.getContextPath()%>/jsp/dms/common/download.jsp?filename=channelSafeStockImp.xls");
    ...
    $('#btn-impPromPart').bind('click', function () {
        ...
        //最后一个参数表示 导入成功后显示页                               
        importXls("PT_BA_CHANNEL_SAFESTOCK_TMP","",5,3,"/jsp/dms/oem/part/basicInfo/channelSafeStocks/importSuccess.jsp");
    });                          
});
...
function doUpdate(rowobj)
{
	...
	$.pdialog.open(webApps+"/jsp/dms/oem/part/basicInfo/channelSafeStocks/channelSafeStockAdd.jsp?action=2", "修改渠道安全库存", "修改渠道安全库存", diaAddOptions);
}
...
<div id="layout" style="width:100%;">
	...
	<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath() %>/images/default/nav.gif" />&nbsp;当前位置：  配件管理  &gt; 基础数据管理   &gt; 基础信息管理   &gt; 渠道安全库存管理</h4>
	<div class="page" >
	<div class="pageHeader" >
			<!-- 提交查询请求form -->
			<form method="post" id="fm-searchChannelSafeStock">
				<div class="searchBar" align="left" >
					<table class="searchContent" id="tab-searchChannelSafeStock">
					<!-- 定义查询条件 -->
					<tr>
						<td><label>渠道商名称：</label></td>
					    <td>
					    	<input type="text" id="orgCode"  name="orgCode" kind="dic" 
					    		src="T#TM_ORG:CODE:ONAME{ORG_ID}:1=1 AND ORG_TYPE='200006' OR ORG_TYPE='200007' AND STATUS='100201'" 
					    		datasource="A.ORG_CODE" datatype="1,is_null,16" />
					    	<input type="hidden" id="orgName"  name="orgName" datasource="A.ORG_NAME"  />
					    </td>
	...
	<div class="pageContent">
		<div id="page_channelSafeStockList" >
			<!-- table参数说明：{ref=父级div的id（page_userlist）,refQuery=与结果列表对应的查询条件table的id} -->
			<table style="display:none;width:100%;" id="tab-channelSafeStockList" name="tablist" ref="page_channelSafeStockList" refQuery="tab-searchChannelSafeStock" >
					<thead>
						<tr>
