			  ...
			  <fieldset>
						<table class="editTable" id="dia-tab-htxx">
							<tr>
			                    ...
			                    <td><input type="text" id="dia-orgCode" name="dia-orgCode" datasource="ORG_CODE" hasBtn="true" callFunction="openSupplier('ORG_CODE');"  datatype="0,is_null,300" readonly="readonly"/></td>
			                    <td><label>渠道名称：</label></td>
			                    <td><input type="text" id="dia-orgName" name="dia-orgName" datasource="ORG_NAME" datatype="0,is_null,3000" readonly="true"/></td>
			                ...
			                <tr>
			                    ...
			                    <td><input type="text" id="dia-TARIFF_NO" name="dia-TARIFF_NO" datasource="TARIFF_NO" datatype="0,is_null,3000" readonly="true"/></td>
			                </tr>
			                <tr>
			                	<td><label>开户银行：</label></td>
			                    <td><input type="text" id="dia-OPEN_BANK" name="dia-OPEN_BANK" datasource="OPEN_BANK" datatype="0,is_null,3000" readonly="readonly"/></td>
			                	<td><label>银行帐号：</label></td>
			                    <td><input type="text" id="dia-BANK_ACCOUNT" name="dia-BANK_ACCOUNT" datasource="BANK_ACCOUNT" datatype="0,is_null,3000" readonly="readonly"/></td>
			                </tr>
			                <tr>
			                	<td><label>地址：</label></td>
			                    <td><input type="text" id="dia-ADDRESS" name="dia-ADDRESS" datasource="ADDRESS" datatype="0,is_null,3000" readonly="readonly"/></td>
			                    <td><label>电话：</label></td>
			                    <td><input type="text" id="dia-TELEPHONE" name="dia-TELEPHONE" datasource="TELEPHONE" datatype="1,is_null,3000" readonly="readonly"/></td>
			                </tr>
			                <tr>
                    ...
                    <div id="dia-part" style="">
                        <table style="display:none;width:100%;" id="dia-tab-partinfo" name="tablist" ref="dia-part" refQuery="tab-searchPart" edit="true">
                           ...
                           <tr>
                                ...
                                <th fieldname="IN_INVOICE_PRICE" colwidth="100" ftype="input" fdatatype="1,is_null,30">含税单价</th>
                                <th fieldname="IN_INVOICE_AMOUNT" colwidth="100" ftype="input" fdatatype="1,is_null,30" >含税金额</th>
                ...
                <div class="formBar">
                    <ul>
                        <li><div class="button"><div class="buttonContent"><button type="button" name="btn-pre">上&nbsp;一&nbsp;步</button></div></div></li>
                         <li id="dia-print"><div class="button"><div class="buttonContent"><button type="button" id="btn-print" >打&nbsp;&nbsp;印</button></div></div></li>
                        <li><div class="button"><div class="buttonContent"><button class="close" type="button">关&nbsp;&nbsp;闭</button></div></div></li>
                    </ul>
                </div>
            </div>
...
/* 取父页面传过的参数确定新增或者修改 */
    ...
    //    $.contractMng.init();
        ...
        $("#btn-save").bind("click", function(event){
			...
			if(diaAction == 1)
			{
				var addUrl = diaUrl + "/entrustInsert.ajax";
        ...
        if (diaAction != "1") {
        	...
        	var accountSearchUrl = diaUrl + "/getOrg.ajax";
    ...
    try {
        ...
        if(rows && rows.length > 0)
        {
            for(var i=0;i<rows.length;i++){
                var flag = getNodeText(rows[i].getElementsByTagName("FLAG").item(0));
                if(flag){
                	$("#dia-orgCode").attr("readonly",false);
                	$("#dia-orgName").attr("readonly",false);
                	$("#dia-TARIFF_NO").attr("readonly",false);
                	$("#dia-OPEN_BANK").attr("readonly",false);
                	$("#dia-BANK_ACCOUNT").attr("readonly",false);
                	$("#dia-ADDRESS").attr("readonly",false);
                	$("#dia-TELEPHONE").attr("readonly",false);
                }else{
                	$("#dia-orgCode").attr("readonly",true);
                	$("#dia-orgName").attr("readonly",true);
                	$("#dia-TARIFF_NO").attr("readonly",true);
                	$("#dia-OPEN_BANK").attr("readonly",true);
                	$("#dia-BANK_ACCOUNT").attr("readonly",true);
                	$("#dia-ADDRESS").attr("readonly",true);
                	$("#dia-TELEPHONE").attr("readonly",true);
                }
            }
        }
    } catch (e) {
	...
	{
		var options = {width:950,height:400,max:false,mask:true,mixable:false,minable:false,resizable:true,drawable:true}; 
		var url = webApps +"/jsp/dms/oem/part/sales/orderMng/entrustOrgSel.jsp";
		$.pdialog.open(url, "supplierSel", "供应商选择", options);
	}
    function SelCallBack(obj)
	{
		$("#dia-orgName").val(obj.getAttribute("ONAME"));//原订单编号
		$("#dia-orgCode").val(obj.getAttribute("CODE"));//原订单编号
		$("#dia-TARIFF_NO").val(obj.getAttribute("TARIFF"));//原订单ID
		$("#dia-OPEN_BANK").val(obj.getAttribute("OPEN_BANK"));//原订单编号
		$("#dia-BANK_ACCOUNT").val(obj.getAttribute("BANK_ACCOUNT"));//原订单编号
		$("#dia-ADDRESS").val(obj.getAttribute("ADDRESS"));//原订单编号
		$("#dia-TELEPHONE").val(obj.getAttribute("BUS_PERSON_TEL"));//原订单编号
	}
