                ...
                <form method="post" id="dia-fm-save" class="editForm" style="width:99%;">
                    <div align="left">
                        <table class="editTable" id="dia-tab-Edit">
                            ...
                            <input type="hidden" id="ifSuper" name="ifSuper" datasource="IF_SUPER"/>
                            <tr>
                            ...
                            <tr style="display: none;">
                                <td>是否渠道订单</td>
                                <td><input type="text" id="ifSuper_Edit" style="width:80px;" name="ifSuper_Edit" kind="dic" src="SF" datasource="IF_SUPER"  datatype="0,is_null,30"/></td>
                                <td id="orgIdSf_Edit1"><label>渠道商：</label></td>
                                <td id="orgIdSf_Edit2">
                                    <input type="text" id="diaOrgName" name="diaOrgName" datasource="ORG_NAME" datatype="0,is_null,10000" readOnly hasBtn="true" callFunction="showOrgTree('orgCode',1,2,2)" operation="="/>
                                    <input type="hidden" id="diaOrgCode"  name="diaOrgCode" datasource="ORG_CODE"/>
                                    <input type="hidden" id="diaOrgId" name="diaOrgId" datasource="ORG_ID"/>
                                </td>
...
<!--                                 </td> -->
                            ...
                            <tr id="trans2" style="display:none;">
                                ...
                                <td><input type="text" id="dia-in-zipCode"  name="dia-in-zipCode" datasource="ZIP_CODE" datatype="0,is_digit,15"/></td>
...
<script type="text/javascript">
    var action = "<%=action%>";
    var diaUrl = "<%=request.getContextPath()%>/part/salesMng/orderMng/TechnologyUpgradesOrderReportCCMngAction";
    $(function(){
        ...
        if (action != "1") {
            ...
            $("#dia-trans-type").attr("readOnly","readOnly");
            // 订单总金额
...
//             $("#diaWarehouseCode").val(selectedRows[0].attr("WAREHOUSE_NAME"));
        	...
        	// 提渠道订单
        	if (selectedRows[0].attr("IF_SUPER")==<%=DicConstant.SF_01%>) {
        		$("#ifSuper_Edit").attr("readOnly","readOnly");
//         		$("#ifSuper_Edit").val("是");
        		$("#orgIdSf_Edit1").show();
	        	$("#orgIdSf_Edit2").show();
...
<%--         		$("#dia-in-custormName").val("<%=orgName%>"); --%>
        	...
        	$("#ifSuper_Edit").attr("code",<%=DicConstant.SF_02%>);
...
//         sendPost(accountSearchUrl, "", "", accountSearchCallback, "false");
        ...
        $("#btnImpOrderPart").bind("click", function () {
            ...
            if ($("#ifSuper_Edit").attr("CODE")==<%=DicConstant.SF_01%>) {
	            importXls("PT_BU_SALE_ORDER_DTL_TMP",$("#diaOrderId").val(),5,3,"/jsp/dms/dealer/part/sales/orderMng/importSuccess.jsp?flag=2");
            } else {
        ...
        $("#btnImp").bind("click", function () {
            importXls("PT_BU_SALE_ORDER_DTL_TMP",$("#diaOrderId").val(),5,3,"/jsp/dms/oem/part/sales/orderMng/importSuccess.jsp");
        });
        ...
        $("#dia-report").bind("click",function(){
            ...
            var reportUrl = diaUrl + "/partOrderReport.ajax?&orderId="+$("#diaOrderId").val()+"&orderAmount="+$("#diaOrderAmount").val()+"&ifSuper="+$("#ifSuper_Edit").attr("code");
...
//             $.pdialog.closeCurrent();
    ...
    // 查询回调方法：改变表格的高度，防止部分不可见;
	function callbackSearch(responseText, tabId){
		
		// 获取数据表格高度 + 表头高度
		var changeHeight = $("#tab-orderPartList_content").height() + 30;
		// 设置显示Div高度
		$("#dia-pjmx").height(changeHeight);
		
		// 此处代码未解决当分页行数量大于当前高度不可见bug
	}
    ...
    function addPjmx() {
        ...
        $.pdialog.open(webApps + "/jsp/dms/oem/part/sales/orderMng/technologyUpgradesOrderPartSearch.jsp", "directBusinessOrderPartSearch", "配件信息查询", options);
    }
