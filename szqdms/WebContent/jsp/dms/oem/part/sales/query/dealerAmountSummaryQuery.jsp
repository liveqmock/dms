...
<body>
	<div id="layout" style="width: 100%;overflow-y: scroll;">
		...
		<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath() %>/images/default/nav.gif" />&nbsp;当前位置： 信息查询  &gt; 财务相关   &gt; 配送中心库存金额汇总表</h4>
		<div class="page">
					...
					<form method="post" id="fm-searchContract">
						...
						<div class="searchBar" align="left" >
							<table class="searchContent" id="tab-htcx">
								<!-- 定义查询条件 -->
								<input type="hidden" id="ORG_ID" name="ORG_ID" datasource="ORG_ID" datatype="0,is_null,30" />
								<tr>
									<td><label>库区代码：</label></td>
									<td>										
										<input type="text" id="ORG_CODE" name="ORG_CODE"  datatype="0,is_null,100" operation="in" readonly="readonly" hasBtn="true" callFunction="open();"/>
									</td>
				...
				<div >
					<table id="invertoryTable">
							...
							<tbody>
								...
								<tr>
									<!-- 期初库存 -->
									<td rowspan="5"  class="showTitle"><span id="INIT_INVENTORY_D"></span></td>
									<td>订单验收入库</td>
									<td class="contentAmount"><span id="CHECK_ORDER_AMOUNT_D"></span></td>
									<td>销售出库</td>
									<td class="contentAmount"><span id="SALES_AMOUNT_D"></span></td>
									<td rowspan="5"  class="showTitle"><span id="END_INVENTORY_D"></span></td>
								</tr>
								<tr>
									<td>销售退件入库</td>
...
<!-- 							<tr >
									<td>价差</td>
									<td class="contentAmount"><span id="PRICE_DIFFERENCE_D"></span></td>
									<td>&nbsp;</td>
									<td>&nbsp;</td>
								</tr> -->
								<tr >
...
$(function(){
	...
	function showOverstockInfo(jsonObj){
		...
		var dataJSON = jsonObj['ROW_0'];
 		...
 		var hJ1 = parseFloat(dataJSON["CHECK_ORDER_AMOUNT_D"]) + parseFloat(dataJSON["SALES_RETURN_AMOUNT_D"]) + parseFloat(dataJSON["SELL_RETURN_AMOUNT_D"]);
 		$("#HJ1").html(hJ1 == 0 ? "0.00" : amountFormatNew(hJ1));
 		
 		// 合计2
 		var hJ2 = parseFloat(dataJSON["SALES_AMOUNT_D"]) + parseFloat(dataJSON["SELL_AMOUNT_D"]);
 		$("#HJ2").html(hJ2 == 0 ? "0.00" : amountFormatNew(hJ2));
 		
 		// 期末库存
 		var qmkc = parseFloat(dataJSON["INIT_INVENTORY_D"]) + hJ1 - hJ2;
 		$("#END_INVENTORY_D").html(qmkc == 0 ? "0.00" : amountFormatNew(qmkc));
