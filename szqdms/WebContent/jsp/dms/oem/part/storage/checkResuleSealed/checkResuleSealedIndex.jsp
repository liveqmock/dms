...
var searchUrl = "<%=request.getContextPath()%>/part/basicInfoMng/PtBuInventoryAction/searchSeale.ajax";
...
$(function(){
	...
	$("#btn-xz").bind("click",function(event){		
		...
		$.pdialog.open(webApps + "/jsp/dms/oem/part/storage/checkResuleSealed/checkResuleSealedAdd.jsp?action=1", "pdxz", "盘点设置新增", options);								  
	});
});
...
function doUpdate(rowobj){
	...
	$.pdialog.open(webApps + "/jsp/dms/oem/part/storage/checkResuleSealed/checkResuleSealedAdd.jsp?action=2", "pdbj", "盘点结果确认", options);
}
...
function toDetail(INVENTORY_ID,link){
	...
	var options = {max:false,width:w,height:600,mask:true,mixable:true,minable:true,resizable:true,drawable:true};
	$.pdialog.open(webApps+"/jsp/dms/oem/part/storage/checkResuleSealed/checkResuleSealedDetail.jsp?INVENTORY_ID="+INVENTORY_ID, "pdmx", "盘点设置明细", options);
}
...
<div id="layout" style="width:100%;">
	...
	<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath() %>/images/default/nav.gif" />&nbsp;当前位置： 仓储管理  &gt; 库存盘点  &gt; 库存盘点财务确认</h4>
	<div class="page" >
			...
			<table style="display:none;width:100%;" id="tab-list" name="tab-list" ref="page_ddlb" refQuery="tab-pkcx" >
					<thead>
						<tr>
							...
							<th colwidth="85" type="link" title="[盘点结果确认]"  action="doUpdate" >操作</th>
						</tr>
					</thead>					
			</table>
		</div>
