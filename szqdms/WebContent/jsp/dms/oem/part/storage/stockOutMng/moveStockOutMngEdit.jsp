        ...
        <div class="tabsContent">
            ...
            <div style="height:auto;overflow:hidden;">
                <form method="post" id="fm-outBillInfo" class="editForm">
                    <%--隐藏域查询条件--%>
                        ...
                        <input type="hidden" id="dia-OUT_TYPE" name="dia-OUT_TYPE" datasource="OUT_TYPE" value="<%=DicConstant.CKLX_03%>"/>
                            ...
                            <table class="editTable" id="tab-outBillInfo">
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <input type="text" id="dia-WAREHOUSE_CODE" name="dia-WAREHOUSE_CODE" datasource="WAREHOUSE_CODE" kind="dic" src="T#PT_BA_WAREHOUSE:WAREHOUSE_CODE:WAREHOUSE_NAME{WAREHOUSE_ID,WAREHOUSE_NAME}:1=1 AND STATUS=<%=DicConstant.YXBS_01 %> AND WAREHOUSE_ID IN (SELECT WAREHOUSE_ID FROM PT_BA_WAREHOUSE WHERE ORG_ID='<%=user.getOrgId()%>') AND OEM_COMPANY_ID = <%=user.getOemCompanyId()%> ORDER BY WAREHOUSE_CODE" datatype="0,is_null,30"/>
                                    </td>
                                    <td><label>目标仓库：</label></td>
                                    <td>
                                        <input type="hidden" id="dia-DES_WAREHOUSE_ID" name="dia-DES_WAREHOUSE_ID" datasource="DES_WAREHOUSE_ID"/>
                                        <input type="hidden" id="dia-DES_WAREHOUSE_NAME" name="dia-DES_WAREHOUSE_NAME" datasource="DES_WAREHOUSE_NAME"/>
                                        <input type="hidden" id="dia-DES_WAREHOUSE_TYPE" name="dia-DES_WAREHOUSE_TYPE" datasource="DES_WAREHOUSE_TYPE"/>
                                        <input type="text" id="dia-DES_WAREHOUSE_CODE" name="dia-DES_WAREHOUSE_CODE" datasource="DES_WAREHOUSE_CODE" kind="dic" src="T#PT_BA_WAREHOUSE:WAREHOUSE_CODE:WAREHOUSE_NAME{WAREHOUSE_ID,WAREHOUSE_NAME,WAREHOUSE_TYPE}:1=1 AND STATUS=<%=DicConstant.YXBS_01 %> AND WAREHOUSE_ID NOT IN (SELECT WAREHOUSE_ID FROM PT_BA_WAREHOUSE WHERE ORG_ID='<%=user.getOrgId()%>') AND WAREHOUSE_TYPE IN (100101,100102,100103,100104,100105,100109,100110,100111) AND OEM_COMPANY_ID = <%=user.getOemCompanyId()%> ORDER BY WAREHOUSE_CODE" datatype="0,is_null,30"/>
                                    </td>
                                </tr>
                                <tr>
                                    <td><label>移库人：</label></td>
                                    <td>
                                        <input type="text" id="dia-MOVE_MAN" name="dia-MOVE_MAN" readonly datasource="MOVE_MAN" datatype="1,is_null,30"/>
                                    </td>
                                    <td><label>移库时间：</label></td>
                                    <td>
                                        <input type="text" id="dia-MOVE_DATE"  name="dia-MOVE_DATE" readonly dataSource="MOVE_DATE" datatype="1,is_null,30" />
                                    </td>
                                </tr>
                                <tr>
            ...
            <div>
                ...
                <div id="div-outBillPartList" style="">
                    <table style="display:none;width:100%;" layoutH="500px" id="tab-outBillPartList" name="tablist" multivals="div-selectedPart" ref="div-outBillPartList" refQuery="tab-searchOutBillPart">
                        <thead>
                        <tr>
                ...
                <div class="formBar">
                    <ul>
                       ...
                       <!--   <li><div class="button"><div class="buttonContent"><button type="button" id="btn-saveDtl">保&nbsp;&nbsp;存</button></div></div></li>-->
                        <li><div class="button"><div class="buttonContent"><button type="button" id="btn-stockOut">出&nbsp;&nbsp;库</button></div></div></li>
...
//     $("#tab-outBillPartList").attr("layoutH",document.documentElement.clientHeight-270);
...
//                         }
    ...
    if (diaAction != "1") {//修改
        ...
        // 目标仓库
        $("#dia-DES_WAREHOUSE_CODE").attr("code",selectedRows[0].attr("DES_WAREHOUSE_CODE"));
        $("#dia-DES_WAREHOUSE_CODE").val(selectedRows[0].attr("DES_WAREHOUSE_NAME"));
        ...
        $('#dia-DES_WAREHOUSE_CODE').attr('datatype','1,is_null,30');

    } else {//新增
        // 移库人
        $("#dia-MOVE_MAN").val("<%=account%>");
        // 移库时间
        $("#dia-MOVE_DATE").val("<%=date%>");
           var url = diaSaveAction+"/getWarehouse.ajax";
        sendPost(url, "", "", getWarehouseCallBack, "false");
//         setDiaDefaultValue();
    }
...
function searchOutBillPart() {
    var searchOutBillPartUrl = "<%=request.getContextPath()%>/part/storageMng/stockOutMng/MoveStockOutMngAction/searchOutBillPart.ajax?OUT_ID=" + $('#dia-OUT_ID').val()+"&WAREHOUSE_ID="+$('#dia-WAREHOUSE_ID').val();
...
function openPartSelWin(){
    ...
    $.pdialog.open(webApps + "/jsp/dms/oem/part/storage/stockOutMng/moveStockOutMngPartSel.jsp", "partSelWin", "配件信息查询", options);
}
...
function afterDicItemClick(id, $row, selIndex){
    ...
    if(id == 'dia-WAREHOUSE_CODE'){
        // 修改目标仓字典
        $("#dia-DES_WAREHOUSE_CODE").attr("src","T#PT_BA_WAREHOUSE:WAREHOUSE_CODE:WAREHOUSE_NAME{WAREHOUSE_ID,WAREHOUSE_NAME,WAREHOUSE_TYPE}:1=1 AND STATUS=<%=DicConstant.YXBS_01 %> AND WAREHOUSE_TYPE IN (100101,100102,100103,100104,100105) AND WAREHOUSE_ID NOT IN (SELECT WAREHOUSE_ID FROM PT_BA_WAREHOUSE WHERE ORG_ID='<%=user.getOrgId()%>') AND OEM_COMPANY_ID = <%=user.getOemCompanyId()%> ORDER BY WAREHOUSE_CODE");
...
//         	outId+=outId.length?"@"+$tr.attr("OUT_ID"):$tr.attr("OUT_ID");
...
//                    }
    ...
    }else
    {
        ...
        }else
        {
            ...
            //将需要提交的内容拼接成json
            sCondition = $f.combined(1) || {};
            doNormalSubmit($f, url, "btn-saveDtl", sCondition, diaUpdateCallBack,"false");
        }
    }
