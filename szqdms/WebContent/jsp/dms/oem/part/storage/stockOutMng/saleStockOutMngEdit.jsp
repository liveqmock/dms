...
var diaSaveAction = "<%=request.getContextPath()%>/part/storageMng/stockOutMng/SaleStockOutMngAction";
...
function searchOutBillPart() {
    var searchOutBillPartUrl = "<%=request.getContextPath()%>/part/storageMng/stockOutMng/SaleStockOutMngAction/searchOutBillPart.ajax?ISSUE_ID=" + $('#dia-ISSUE_ID').val()+"&account="+$('#PERSON_NAME').attr('code');
...
function createInputBox3(obj)
{
    var $obj = $(obj);
    var $tr = $(obj).parent();
    var default_no = $tr.find("td").eq(8).text();
    var outAmount = $tr.find("td").eq(12).text();
    var $checkbox = $tr.find("td").eq(1).find("input[type='checkbox']:first");
...
function doMyInputCount(obj){
    ...
    var o = $obj.val();
    var s = $tr.find("td").eq(8).text();
    if(parseInt(o)-parseInt(s)>0){
        alertMsg.warn('出库数量不能大于应发数量.');
        $obj.val(s);
        return false;
    }
    if($obj.val() > 0)
