...
<!-- 							<tr>
							...
							<tr>
								...
								<td colspan="3">
									<input type="text" id="ORDER_AMOUNT_D" style="text-align: right;" /> 
								</td>
							</tr>
						</table>
					</div>
...
$(function(){
	var id = "<%=id%>";	// 主信息ID
	var getDetailsURL = "<%=request.getContextPath()%>/part/salesMng/orderMng/BoxInfoDealerQueryAction/queryBoxInfoById.ajax"; // 查询ActionURL
	...
	function showApplicationInfo(jsonObj){
		...
		// 格式化金额
		$("#ORDER_AMOUNT_D").val(amountFormatNew($("#ORDER_AMOUNT_D").val()));
		
		// 调用显示主信息的方法
		$("#btn-search-details-index").click(function(){
			var searchURL = "<%=request.getContextPath()%>/part/salesMng/orderMng/BoxInfoDealerQueryAction/queryDetailsListInfoById.ajax";
