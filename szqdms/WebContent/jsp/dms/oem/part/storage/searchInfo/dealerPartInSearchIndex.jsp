...
$(function(){
	if(orgType==<%=DicConstant.ZZLB_09%>){
		$("#STORAGE_TYPE").attr("filtercode","<%=DicConstant.QDCRKLX_02%>|<%=DicConstant.QDCRKLX_04%>|<%=DicConstant.QDCRKLX_06%>");
	}else{
		$("#STORAGE_TYPE").attr("filtercode","<%=DicConstant.QDCRKLX_02%>");
	}
	$("#btn-search").click(function(){
		var queryAction = "<%=request.getContextPath()%>/part/storageMng/search/DealerPartOutQueryAction/partInSearch.ajax";
	...
	$("#btn-export-index").click(function(){
        ...
        sCondition = $f.combined() || {};
        $("#data").val(sCondition);
        var url = encodeURI("<%=request.getContextPath()%>/part/storageMng/search/DealerPartOutQueryAction/partInDownload.do");
...
<div id="layout" style="width:100%;">
	...
	<h4 class="contentTitle" align="left"><img src="<%=request.getContextPath() %>/images/default/nav.gif" />&nbsp;当前位置： 渠道信息查询  &gt; 仓储相关   &gt; 配件入库查询</h4>
	<div class="page" >
			...
			<div class="searchBar" align="left" >
				<table class="searchContent" id="tab-htcx">
					...
					<tr>
					    ...
					    <td><input type="text" id="OUT_NO" name="OUT_NO" datatype="1,is_null,3000" dataSource="T.IN_NO" operation="like" /></td>
			...
			<table style="display:none;width:100%;" id="invertoryTable" name="tablist" ref="page_contract" refQuery="fm-searchContract" >
					<thead>
						<tr>
							...
							<th fieldname="ORG_CODE">发/退货方代码</th>
							<th fieldname="ORG_NAME">发/退货方名称</th>
							<th fieldname="IN_NO" >单号</th>
							<th fieldname="STORAGE_TYPE" >入库类型</th>
							<th fieldname="COUNT" >数量</th>
							<th fieldname="PRICE" >单价</th>
							<th fieldname="AMOUNT" >金额</th>
							<th fieldname="APPLY_DATE" >入库日期</th>
						</tr>
					</thead>
					<tbody>
                    </tbody>
