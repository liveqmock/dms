...
var mngUrl = "<%=request.getContextPath()%>/part/storageMng/search/DealerPartStockInOrOutSearchMngAction";
$(function()
{
...
function doSearchCallBack(res){
    ...
    if(rows && rows.length > 0){
         ...
         var $tde = $("<td></td>");
         $row0.append($tde);
         var amount = getNodeText(rows[0].getElementsByTagName("BALANCE_COUNT").item(0));
         ...
         if(orderId=='1'){
        	 for(var i=1;i<rows.length;i++){
            	 ...
            	 var $td1 = $("<td>"+getNodeText(rows[i].getElementsByTagName("STOCK_DATE").item(0))+"</td>");
             	 $row.append($td1);
             	 /* var $td2 = $("<td><font size='4' color='red'><a href='javascript:void(0);' onclick='doDetail("+getNodeText(rows[i].getElementsByTagName("ORDER_ID").item(0))+","+getNodeText(rows[i].getElementsByTagName("ORDER_NO").item(0))+","+getNodeText(rows[i].getElementsByTagName("TYPE").item(0))+")'>"+getNodeText(rows[i].getElementsByTagName("ORDER_NO").item(0))+"</a><fond></td>"); */
            	 var $td2 = $("<td ondblclick='doDblclick(this)'><font size='4' color='red'><input type='hidden' name = 'id' id='ID' value="+getNodeText(rows[i].getElementsByTagName("ORDER_NO").item(0))+"></input>"+getNodeText(rows[i].getElementsByTagName("ORDER_NO").item(0))+"<fond></td>");
             	 $row.append($td2);
            	 var $td3 = $("<td>"+getNodeText(rows[i].getElementsByTagName("IN_COUNT").item(0))+"</td>");
            	 $row.append($td3);
            	 var $td4 = $("<td>"+getNodeText(rows[i].getElementsByTagName("OUT_COUNT").item(0))+"</td>");
            	 $row.append($td4);
            	 /* var $td5 = $("<td>"+getNodeText(rows[i].getElementsByTagName("BALANCE_COUNT").item(0))+"</td>"); */
            	 inCount = parseInt(inCount) +parseInt(getNodeText(rows[i].getElementsByTagName("IN_COUNT").item(0)));
            	 outCount = parseInt(outCount) + parseInt(getNodeText(rows[i].getElementsByTagName("OUT_COUNT").item(0)));
...
function openOutDetailPage(outId){
    ...
    $.pdialog.open(webApps+"/jsp/dms/oem/part/storage/searchInfo/dealerOrderOfPartOut.jsp?ORDER_NO="+outId, "outWin", "出库单详细信息", Ooptions);
}
function openInDetailPage(inId){
	var Ioptions = {max:true,mask:true,mixable:true,minable:true,resizable:true,drawable:true};
    $.pdialog.open(webApps+"/jsp/dms/oem/part/storage/searchInfo/dealerOrderOfPartIn.jsp?ORDER_NO="+inId, "inWin", "入库单详细信息", Ioptions);
} 
</script>