...
<head>
    ...
    $(function(){
        $('#info').append('<tr><td>客户: '+opener.$('#dia-ORG_NAME').val()+'</td><td></td><td style="text-align: right">编号: '+opener.$('#dia-ISSUE_NO').val()+'</td></tr><tr><td>库管员: '+opener.$('#dia-USER_NAME').val()+'</td><td>销售员: '+opener.$('#dia-CHECK_USER').val()+'</td><td style="text-align:left">打印日期: <%=today%></td></tr><tr></tr>');
        var queryUrl = "<%=request.getContextPath()%>/part/storageMng/stockIssueMng/PrintIssueAction/searchIssueDtl.ajax?ISSUE_ID="+opener.$('#dia-ISSUE_ID').val()+"&printFlag=1";
    ...
    function queryCallback(ret){
        ...
        if (rows && rows.length > 0) {
            for(var i = 0;i<rows.length;i++){
                ...
                var SHOULD_COUNT = getNodeText(rows[i].getElementsByTagName("SHOULD_COUNT").item(0));
                var REAL_COUNT = getNodeText(rows[i].getElementsByTagName("REAL_COUNT").item(0));
               ...
               /*  var R_COUNT = "";
                ...
                $('#partList').find('tbody').append('<tr><td>'+(i+1)+'</td><td>'+PART_CODE+'</td><td>'+PART_NAME+'</td><td>'+UNIT+'</td><td>'+a+'</td><td>'+POSITION_CODE+'</td><td>&nbsp;'+SHOULD_COUNT+'&nbsp;</td><td>&nbsp;'+REAL_COUNT+'</td><td>&nbsp;'+REMARK+'&nbsp;</td></tr>');
            }
        }
    }
