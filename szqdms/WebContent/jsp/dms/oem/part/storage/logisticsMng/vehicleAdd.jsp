...
<div id="dia-layout">
	...
	<form method="post" id="fm-vehicle" class="editForm" >
	    <!-- 隐藏域 -->
		...
		<div align="left">
			<fieldset>
			<table class="editTable" id="tab-vehicleInfo">
				<tr>
					<td><label>VIN：</label></td>
				    <td>
				    	<input type="text" id="dia-vin"  name="dia-vin" datasource="VIN" datatype="0,is_vin,17" />
				    </td>
				    <td><label>发动机号：</label></td>
				    <td>
				    	<input type="text" id="dia-engineNO"  name="dia-engineNO" datasource="ENGINE_NO" datatype="0,is_fdjh,30" />
				    </td>
				...
				<tr>
				    ...
				    <td>
				    	<input type="text" id="dia-licensePlate"  name="dia-licensePlate" datasource="LICENSE_PLATE" datatype="0,is_carno,30" />
				    </td>
				</tr>
				<tr>
					<td><label>车辆类型：</label></td>
				    <td>
				    	<select class="combox" id="dia-vehicleType" name="dia-vehicleType" kind="dic" src="CYCX" datasource="VEHICLE_TYPE" datatype="0,is_null,6">
 ...
 * 保存样例
 */
var diaSaveAction = "<%=request.getContextPath()%>/part/storageMng/logisticInfo/VehicleAction";
...
$(function(){ 
	...
	{ 
		...
		var selectedRows = $("#tab-vehicleList").getSelectedRows();
		setEditValue("fm-vehicle",selectedRows[0].attr("rowdata"));
...
function diaInsertCallBack(res)
{
	try
	{
		//第二个参数0表示插入到第几行
		//alert(res.xml);
		$("#tab-vehicleList").insertResult(res,0);
	}catch(e)
	{
		alertMsg.error(e);
		return false;
...
function diaUpdateCallBack(res)
{
	try
	{   
		var selectedIndex = $("#tab-vehicleList").getSelectedIndex();
		$("#tab-vehicleList").updateResult(res,selectedIndex);
