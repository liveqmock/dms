					...
					<table class="searchContent">
						...
						<tbody>
							<tr>
								...
								<td><input type="text" class="textInput"style="margin: 0px" name="PART_NO" /><span style='color:red'>*</span></td>
								<td><input type="text" class="textInput"style="margin: 0px" name="PART_NAME"/><span style='color:red'>*</span></td>
...
<form id="applicationForm" style="display:none" method="post">
	...
	<!-- 配件编号 -->
	<input type="hidden" name="PART_NO_S_A" id="PART_NO_S_A" datasource="PART_NO_S_A" val="" />
	<!-- 配件名称 -->
	<input type="hidden" name="PART_NAME_S_A" id="PART_NAME_S_A" datasource="PART_NAME_S_A" value="" />
	<!-- 车辆识别码 -->
	<input type="hidden" name="VIN_S_A" id="VIN_S_A" datasource="VIN_S_A" value="" />
	<!-- 配件信息录入子表ID -->
...
function validateSubpage(){
	var result = true;
	var partNoStr = "";
	var inputArray = $("input[type=text]", "tr");
...
function initSaveSubpageData(){
	...
	inputArray.each(function(index, inputObj){
		...
		if($(inputObj).attr("name") == "PART_NO"){
			...
			pageNameStr += inputVal + ",";
		} else if($(inputObj).attr("name") == "VIN"){
			vinStr += (inputVal == "" ? "null" : inputVal) + ",";
		}
	})
	partNoStr = partNoStr.substring(0, partNoStr.length - 1);
	pageNameStr = pageNameStr.substring(0, pageNameStr.length - 1);
	vinStr = vinStr.substring(0, vinStr.length - 1);
	
	// 将获取的值保存至隐藏的form表单中
	$("#PART_NO_S_A").val(partNoStr);
	$("#PART_NAME_S_A").val(pageNameStr);
	$("#VIN_S_A").val(vinStr);
}
