...
<script type="text/javascript">
    var orgType = '<%=orgType%>';
    var searchUrl = "<%=request.getContextPath()%>/part/partClaimMng/claimCycl/ClaimCyclApplyMngAction/searchInStockOrder.ajax";
    // 弹出窗体
    var dialog = $("body").data("searchInStockOrder");
    // 初始化函数
    $(function() {
        ...
        if(orgType=="<%=DicConstant.ZZLB_10%>"||orgType=="<%=DicConstant.ZZLB_11%>"){
            $("#orderType").attr("filtercode","<%=DicConstant.DDLX_01%>|<%=DicConstant.DDLX_02%>|<%=DicConstant.DDLX_03%>|<%=DicConstant.DDLX_04%>|<%=DicConstant.DDLX_06%>|<%=DicConstant.DDLX_07%>");
        }
    ...
    function doRadio(res){
        ...
        // 入库单查询
        // 入库单ID
        $("#inStockOrderId-Edit").val(selectedRows[0].attr("ORDER_ID"));
        orderId = selectedRows[0].attr("ORDER_ID");
        // 入库单号
        $("#inStockOrderNo-Edit").val(selectedRows[0].attr("ORDER_NO"));
        // 清空配件信息
        clearPart();
        // 关闭窗口
