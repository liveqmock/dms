					...
					<table class="searchContent">
						...
						<thead>
							<tr>
								...
								<th width="120">原零件名称</th>
								<th width="120">车辆识别码</th>
								<th width="120">更新编号</th>
								<th width="120">更新名称</th>
								<th width="120">更新（禁用）原因</th>
								<th width="120">技术科意见</th>
								<th>采购科意见</th>
							</tr>
						</thead>
						<tbody>
...
<form id="applicationForm" style="display:none" method="post">
	...
	<!-- 采购科意见-->
	<input type="hidden" name="PURCHASING_DEPARTMENT_REMARK_S_A" id="PURCHASING_DEPARTMENT_REMARK_S_A" datasource="PURCHASING_DEPARTMENT_REMARK_S_A"  />
</form>
...
function validateSubpage(){
	...
	inputArray.each(function(index, inputObj){
		...
		if($.trim(inputVal) == ""){
			var message = "";
			if($(inputObj).attr("name") == "PURCHASING_DEPARTMENT_REMARK"){
				message = "请填写采购科意见";
			} 
...
function initSaveSubpageData(){
	...
	pdRemark = pdRemark.substring(0, pdRemark.length - 1);

	// 将获取的值保存至隐藏的form表单中
	$("#ENTRY_ID_S_A").val(entryIdStr);
	$("#PURCHASING_DEPARTMENT_REMARK_S_A").val(pdRemark);
}
...
$(function(){
	...
	function showApplicationInfo(jsonObj){
		...
		var trHtmlStr = "<tr>"+
						...
						"	<td><input type='text' class='textInput readonly' style='margin: 0px;width:120px;' name='OLD_PART_NAME'  readonly='readonly'  /></td>"+
						"	<td><input type='text' class='textInput readonly' style='margin: 0px;' name='VIN' readonly='readonly' /></td>"+
						"	<td><input type='text' class='textInput readonly' style='margin: 0px;width:120px;' name='NEW_PART_NO'   readonly='readonly' /></td>"+
						"	<td><input type='text' class='textInput readonly' style='margin: 0px;width:120px;' name='NEW_PART_NAME'  readonly='readonly' /></td>"+
						"	<td><input type='text' class='textInput readonly' style='margin: 0px;width:120px;' name='REMARK'  readonly='readonly' /></td>"+
						"	<td><input type='text' class='textInput readonly' style='margin: 0px;width:120px;' name='ENGINEERING_DEPARTMENT_REMARK'  readonly='readonly' /></td>"+
						"	<td><input type='text' class='textInput' style='margin: 0px;' name='PURCHASING_DEPARTMENT_REMARK'  requiredInput='y'/><span style='color:red;'>*</span></td>"+
						"</tr>";
