					...
					<div class="searchBar" align="left">
						<table class="searchContent" id="applicationDetailsTables">
							...
							<tr>
								...
								<td><input type="text" id="PURCHASING_DEPARTMENT_D" name="PURCHASING_DEPARTMENT_D"  dataSource="PURCHASING_DEPARTMENT_D" value="<%=user.getPersonName() %>" readonly="readonly"/></td>
								<td><label>审批结果</label></td>
								<td>
									<select type="text" id="APPLICATION_STATUS_AUDIT_D" name="APPLICATION_STATUS_AUDIT_D"  datasource="APPLICATION_STATUS_AUDIT_D"  datatype="0,is_null,100">
										<option value="-1" selected="selected">--</option>
										<option value="<%=DicConstant.PJWHSQZT_05%>">审核通过</option>
										<option value="<%=DicConstant.PJWHSQZT_06%>">审核驳回</option>
									</select>	
								</td>
								<td><label>采购科审批备注</label></td>
								<td><input type="text" id="PURCHASING_DEPARTMENT_REMARK_D" name="PURCHASING_DEPARTMENT_REMARK_D"  dataSource="PURCHASING_DEPARTMENT_REMARK_D"  datatype="0,is_null,100"/></td>
							</tr>
						</table>
						<div class="subBar">
							<ul>
...
$(function(){
	...
	function showApplicationInfo(jsonObj){
		...
		switch(jsonObj["APPLICATION_TYPE_CODE_D"]){
			case "307201": // 307201 零件编号录入审核
				...
				iframeSrc = "partInfoPDAudit.jsp";
				break;
			case "307202": // 307202 驾驶室总成录入
				iframeSrc = "cabAssemblyPDAudit.jsp";
				$("#VIN_D").removeAttr("readonly").removeClass("readonly");
				break;
			case "307203": // 307203 驾驶室本体录入
				iframeSrc = "cabInfoPDAudit.jsp";
				break;
			case "307204": // 307204 零件编号变更(禁用)
				iframeSrc = "partInfoChangePDAudit.jsp";
				break;
			case "307205": // 307205 供货商变更(禁用)
				iframeSrc = "supplierChangePDAudit.jsp";
				break;
			default:
				iframeSrc = "";
				break;
	...
	function validatePage(){
		if($.trim($("#PURCHASING_DEPARTMENT_D").val()) == ""){
		...
		if($.trim($("#PURCHASING_DEPARTMENT_REMARK_D").val()) == ""){
			alertMsg.warn("请填写采购科审批备注");
			return false;
		}
		return true;
	}
