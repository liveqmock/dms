...
<form id="applicationForm" style="display:none" method="post">
	...
	<!-- 技术科科长意见 -->
	<input type="hidden" name="REMARK_S_A" id="REMARK_S_A" datasource="REMARK_S_A" />
</form>
...
function validateSubpage(){
	...
	if(auditStatus == <%=DicConstant.PJWHSQZT_03%>){
		var inputArray = $("input[type=text][requiredInput=y]", ".searchContent tbody tr");
		inputArray.each(function(index, inputObj){
			var inputVal = $(inputObj).val();
			if($.trim(inputVal) == ""){
				if($(inputObj).attr("name") == "REMARK"){
					message = "请填写技术科科长意见";
				}
...
function initSaveSubpageData(){
	var remarkStr = "";
	var entryIdStr = "";
	var inputArray = $("input[requiredInput=y]", ".searchContent tbody");
	inputArray.each(function(index, inputObj){
...
$(function(){
	...
	var getDetailsURL = "<%=request.getContextPath()%>/part/mainTenanceApplication/MainTenanceApplicationAction/applicationCABInfoDetailsForEdit.ajax";
	...
	function showApplicationInfo(jsonObj){
		...
		var trHtmlStr = "<tr>"+
						"	<td></td>"+
						"	<td><input type='hidden' name='ENTRY_ID' requiredInput='y'/><input type='text' class='textInput readonly' style='margin: 0px;' name='CAB_NO' readonly='readonly'/></td>"+
						"	<td><input type='text' class='textInput readonly' style='margin: 0px;' name='CAB_DESCRIPTION'  readonly='readonly' /></td>"+
						"	<td><input type='text' class='textInput readonly' style='margin: 0px;' name='VIN'  readonly='readonly'  /></td>"+
						"	<td><input type='text' class='textInput' style='margin: 0px;' name='REMARK'  requiredInput='y' /><span style='color:red;'>*</span></td>"+
						"</tr>";
		...
		$(".searchContent tbody tr").each(function(index, obj){
