...
function addTr(){
	var trHtml =    "<tr>"+
					...
					"	<td><input type='text' class='textInput' style='margin: 0px;' name='OLD_PART_NO'/><span style='color:red'>*</span></td>"+
					"	<td><input type='text' class='textInput' style='margin: 0px;' name='OLD_PART_NAME'/><span style='color:red'>*</span></td>"+
					"	<td><input type='text' class='textInput' style='margin: 0px;' name='VIN'/></td>"+
					"	<td><input type='text' class='textInput' style='margin: 0px;' name='NEW_PART_NO' /></td>"+
					"	<td><input type='text' class='textInput' style='margin: 0px;' name='NEW_PART_NAME' /></td>"+
					"	<td><input type='text' class='textInput' style='margin: 0px;' name='REMARK'/><span style='color:red'>*</span></td>"+
					"</tr>"
	...
	if(inputArray.size() == 0){
		...
		inputArray.each(function(index, inputObj){
			...
			if($.trim(inputVal) == ""  && $(inputObj).attr("name") != "VIN"){
				var message = "";
				if($(inputObj).attr("name") == "OLD_PART_NO"){
					message = "请填写原零件编号";
					result = false;
				} else if($(inputObj).attr("name") == "OLD_PART_NAME"){
					message = "请填写原零件名称";
					result = false;
				} else if($(inputObj).attr("name") == "REMARK"){
					message = "请填写更新（禁用）原因";
					result = false;
				}
				...
				var $NEW_PART_NO = $("input[name=NEW_PART_NO]",trObj);
				var $NEW_PART_NAME = $("input[name=NEW_PART_NAME]",trObj);
				...
				$(inputObj).removeClass("errorInput");
				// 零件编号校验
				if($(inputObj).attr("name") == "OLD_PART_NO" || $(inputObj).attr("name") == "NEW_PART_NO"){
...
function initSaveSubpageData(){
	...
	inputArray.each(function(index, inputObj){
		...
		if($(inputObj).attr("name") == "OLD_PART_NO"){
			...
			vinStr += inputVal + ",";
		} else if($(inputObj).attr("name") == "NEW_PART_NO"){
			newPartNo += inputVal + ",";
		} else if($(inputObj).attr("name") == "NEW_PART_NAME"){
