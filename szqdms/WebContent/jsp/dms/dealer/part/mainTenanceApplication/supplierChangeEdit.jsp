...
function addTr(){
	var trHtml =    "<tr>"+
					"	<td><input type='checkbox' style='margin: 0px' onclick='checkBoxClick(this)'/></td>"+
					"	<td><input type='text' class='textInput' name='PART_NO'/></td>"+
					"	<td><input type='text' class='textInput' name='PART_NAME'/></td>"+
					"	<td><input type='text' class='textInput' name='OLD_SUPPLIER' /></td>"+
					"	<td><input type='text' class='textInput' name='NEW_SUPPLIER' /></td>"+
					"	<td><input type='text' class='textInput' name='REMARK'/></td>"+
					"	<td><input type='text' class='textInput' name='OLD_SUPPLIER_REMARK'/></td>"+
					"	<td><input type='text' class='textInput' name='NEW_SUPPLIER_REMARK'/></td>"+
					"</tr>"
...
function validateSubpage(){
	...
	if(inputArray.size() == 0){
		...
		inputArray.each(function(index, inputObj){
			...
			if($.trim(inputVal) == "" && $(inputObj).attr("name") != "NEW_SUPPLIER"){
				var message = "";
				if($(inputObj).attr("name") == "PART_NO"){
					message = "请填写零件编号";
				} else if($(inputObj).attr("name") == "PART_NAME"){
					message = "请填写零件名称";
				} else if($(inputObj).attr("name") == "OLD_SUPPLIER"){
					message = "请填写原供货商";
				} else if($(inputObj).attr("name") == "REMARK"){
					message = "请填写新增（更新）原因";
				} else if($(inputObj).attr("name") == "OLD_SUPPLIER_REMARK"){
					message = "请填写原厂家采购主管意见";
				} else if($(inputObj).attr("name") == "NEW_SUPPLIER_REMARK"){
...
function initSaveSubpageData(){
	...
	inputArray.each(function(index, inputObj){
		...
		} else if($(inputObj).attr("name") == "NEW_SUPPLIER"){
