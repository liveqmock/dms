					...
					<div class="searchBar" align="left">
						<table class="searchContent">
							...
							<tr>
								...
								<td>
									<select type="text" id="APPLICATION_STATUS" name="APPLICATION_STATUS"  datasource="APPLICATION_STATUS"
							...
							<tr>
								...
								%>	
										filtercode="<%=DicConstant.PJWHSQLX_04%>" 
								<%
									}else if(orgName.indexOf("保障科") != -1){
										
								%>
										filtercode="<%=DicConstant.PJWHSQLX_02%>|<%=DicConstant.PJWHSQLX_03%>" 
								<%
									} else {
								%>
										filtercode="<%=DicConstant.PJWHSQLX_01%>|<%=DicConstant.PJWHSQLX_02%>|<%=DicConstant.PJWHSQLX_03%>|<%=DicConstant.PJWHSQLX_04%>" 
								<%		
									}
								%>
									>
					...
					<table style="display:none; width: 100%;" id="tab-contract" name="tablist" ref="page_contract" refQuery="fm-searchContract">
						<thead>
							<tr>
								...
								<th fieldname="APPLICATION_TYPE" colwidth="90px">申请单类型</th>
...
$(function(){
    ...
    $("#btn-add").click(function(){
        ...
        $.pdialog.open(webApps+"/jsp/dms/dealer/part/mainTenanceApplication/mainTenanceApplicationForAdd.jsp", "addApplication", "申请单新增", options);
    });
});
...
function doEdit(rowObj){
	...
	if(status == "<%=DicConstant.PJWHSQZT_01%>"
		...
		var options = {max:true,mask:true,mixable:true,minable:true,resizable:true,drawable:true};
	    $.pdialog.open(webApps+"/jsp/dms/dealer/part/mainTenanceApplication/mainTenanceApplicationForEdit.jsp", "forEditPage", "申请单详情", options);
	}else{
