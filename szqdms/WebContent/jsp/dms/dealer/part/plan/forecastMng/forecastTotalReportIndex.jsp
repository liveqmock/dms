...
<script type="text/javascript">
    // URL
    var indexSearchUrl = "<%=request.getContextPath()%>/part/planMng/forecast/ForecastTotalMngAction/searchForecast.ajax";
    // 初始化方法
     $(function(){
        ...
        $("#btn-totalEdit").click(function(){
        	var options = {max:true,mask:true,mixable:true,minable:true,resizable:true,drawable:true};
             $.pdialog.open(webApps+"/jsp/dms/dealer/part/plan/forecastMng/forecastTotalReportSearch.jsp", "forecastTotalReportSearch", "新增汇总", options);
        });
    });

    // 查询预测汇总
    function forecastTotalReport() {
        var $f = $("#fm-condition");
        var sCondition = {};
    ...
    function doUpdate(rowobj){
        $("td input:first",$(rowobj)).attr("checked",true);
        var options = {max:true,mask:true,mixable:true,minable:true,resizable:true,drawable:true};
        $.pdialog.open(webApps+"/jsp/dms/dealer/part/plan/forecastMng/forecastTotalReportEdit.jsp?action=2", "totalEdit", "预测上报编辑", options);
    }
</script>
</head>
<body>
