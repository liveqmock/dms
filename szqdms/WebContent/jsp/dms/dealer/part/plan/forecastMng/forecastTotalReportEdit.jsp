...
<div id="di_ychzdz" style="width:100%;">
    ...
    <div class="pageContent" style="" >
        ...
        <table style="display:none;width:100%;" id="tab-grid-Edit" multivals="div-selectedPart-edit" name="tablist" ref="edit_page_grid" refQuery="tab-condition">
                <thead>
                    <tr>
...
<script type="text/javascript">
    var action=<%=action%>;
    // URL
    var searchUrl = "<%=request.getContextPath()%>/part/planMng/forecast/ForecastTotalMngAction/searchForecastDetailByMonth.ajax";
    // 弹出窗体
    var dialog = $("body").data("totalEdit");
    $(function() {
    	$("#tab-grid-Edit").attr("layoutH",document.documentElement.clientHeight-270);
        // 修改操作
        if(action != "1"){
            ...
            // 预测月份赋值
            $("#edit-forecastMonth").val($("#month").val());
        }
        // 查询预测配件明细
        searchPromPart();
        ...
        $("#dia-btn-search").bind("click", function (event) {
            ...
            $.pdialog.open(webApps+"/jsp/dms/dealer/part/plan/forecastMng/forecastTotalPartSearch.jsp", "partSearch", "预测配件信息", options);
       });
        // 提报按钮
        $("#doApplyTotal").click(function(){
        	if($("#tab-grid-Edit_content").find("tr").size()==0){
                alertMsg.warn("请添加配件.");
                return false;
            } else if($("#tab-grid-Edit_content").find("tr").size() == 1) {
                if($("#tab-grid-Edit_content").find("tr").eq(0).find("td").size() == 1) {
                    alertMsg.warn("请添加配件.");
                    return false;
                }
            }
            var doApplyTotalUrl = "<%=request.getContextPath()%>/part/planMng/forecast/ForecastTotalMngAction/updateForecast.ajax";
