...
<div  style="width:100%;">
    ...
    <br />
     ...
     <div class="searchBar" align="left" >
    	<form id="tempUmsaveForm" method="post">
			<%--积压件申请单ID --%>
			<input type="hidden" name="overstockId" id="overstockId_f" datasource="overstockId" val=""/>
			<input type="hidden" name="overstockApplyStatus" id="overstockApplyStatus" datasource="overstockApplyStatus" val=""/>
			<table class="searchContent">
				<tr>
                    <td align="right" width="100"><label>申请确认：</label></td>
                    <td>
                    	<label for="applyStatus_f_1" style="float: left;">同意</label>
                    	<input type="radio" name="applyStatus_f" id="applyStatus_f_1" style="margin-top: 3px;" value="<%=DicConstant.JYJSQZT_03%>"/>
                    	<label for="applyStatus_f_2" style="float: left;">驳回</label>
                    	<input type="radio" name="applyStatus_f" id="applyStatus_f_2" style="margin-top: 3px;" value="<%=DicConstant.JYJSQZT_04%>"/>
                    </td>
                </tr>
				<tr>
                    <td align="right"><label>备注：</label></td>
                    <td>
                    	<textarea style="width:300px;" rows="3" name="confirmRemark_f" id="confirmRemark_f" datasource="confirmRemark_f"></textarea>
                    </td>
                </tr>
			</table>
			 <div class="subBar">
    ...
    function validForm(){
    	...
    	var validStr = is_noquotation($("#confirmRemark_f"));
    	if(!(validStr === true)){
    		alertMsg.error(validStr);
    		return false;
    	}
    	
    	// 如果用户选择审核通过,则校验数量：申请数量 是否 大于 库存可用数量，如果大于则不能通过
    	var applyStatus = $("input[name='applyStatus_f']:checked").val();

    	if(applyStatus == <%=DicConstant.JYJSQZT_03%>){
    		$("#fm-part-info-show-table_content tr").each(function(index,trObj){
    			// 比较数量需要转为数字后比较
    			if(parseFloat($(trObj).attr("APPLY_COUNTS")) > parseFloat($(trObj).attr("AVAILABLE_AMOUNT"))){
    				result = false;
    				message = "配件代码：" + $(trObj).attr('PART_CODE') + "，配件名称：" + $(trObj).attr('PART_NAME') + ",库存数量小于申请数量，不能通过确认";
    				return false;
    			}
	   		})
    	}
