...
<%--             srcVal = "T#TM_ORG:CODE:ONAME{ORG_ID,ONAME}:1=1 AND ORG_ID IN(SELECT DC_ID FROM PT_BA_SERVICE_DC WHERE STATUS=<%=DicConstant.YXBS_01 %> AND ORG_ID=<%=user.getOrgId() %> )"; --%>
    ...
    function searchReturnPurchaseApply() {
        var searchUrl = "<%=request.getContextPath()%>/part/salesMng/returnPurchaseMng/ReturnPurchaseApplyMngAction/searchReturnPurchaseApply.ajax";
        var $f = $("#fm-index");
        var sCondition = {};
    ...
    function doUpdate(rowobj) {
        ...
        var options = {max:true,mask:true,width:300,height:150,mixable:true,minable:true,resizable:true,drawable:true};
        $.pdialog.open(webApps + "/jsp/dms/dealer/part/sales/returnPurchaseMng/returnPurchaseApplyEdit.jsp?action=2", "returnPurchaseApplyEdit", "退件申请维护", options);
    }
	var $row;
    // 删除方法
    function doDelete (rowObj) {
        $row = $(rowObj);
        var deleteForecastUrl = "<%=request.getContextPath()%>/part/salesMng/returnPurchaseMng/ReturnPurchaseApplyMngAction/deleteReturnPurchaseApply.ajax";
        var url = deleteForecastUrl + "?returnId=" + $(rowObj).attr("RETURN_ID");
        sendPost(url, "", "", doDeleteCallBack1, "true");
    }

    //删除回调方法
    function doDeleteCallBack1(res) {
        try {
            if ($row)
            ...
            <div class="searchBar" align="left" >
                <table class="searchContent" id="tab-ddcx">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" id="returnCode-index" name="returnCode-index" datasource="RECEIVE_ORG_CODE" datatype="1,is_null,30" operation="like" dicWidth="500" isreload="true" src="" kind="dic"/>
