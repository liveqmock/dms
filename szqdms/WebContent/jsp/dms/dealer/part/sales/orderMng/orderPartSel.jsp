                ...
                <table style="display:none;width:100%;" id="tab-partList" multivals="div-selectedPart" name="tablist" ref="div-partList" refQuery="tab-searchPart">
                    <thead>
                        <tr>
                            ...
                            <th fieldname="SALE_PRICE" colwidth="60" refer="formatAmount" align="right">经销商价</th>
                    ...
                    <table style="width:100%;">
                        <tr>
                            <td>
                                ...
                                <textarea style="width:80%;height:10px;display: none" id="val9" name="multivals" readOnly></textarea>
                            </td>
                        </tr>
                    </table>
                </div>
...
<script type="text/javascript">
    ...
    function diaAddPartSearch(){
        var searchPartUrl = "<%=request.getContextPath()%>/part/salesMng/orderMng/PartOrderReportAction/orderPartAddSearch.ajax?orderId="+$("#diaOrderId").val()+"&warehouseId="+$("#diaWarehouseId").val()+"&orderType="+$("#dia-orderType").val()+"&promId="+$("#diaPromId").attr("code")+"&directTypeId="+$("#diaDirectTypeId").val()+"&belongAssembly="+$("#dia-belong_assembly").val();
        var $f = $("#fm-searchPart");
        var sCondition = {};
        ...
        $('#btn-confirmPart').bind('click',function(){
            ...
            if(!keyIds){
            	...
            	if (planPriceCheck.length>0) {
            		var errorMsg = "请联系管理员维护配件经销商价:"+planPriceCheck;
    ...
    function doMyInputBlur(obj) {
        ...
        // 是配送中心并且不是总成订单时
        if(orgType==<%=DicConstant.ZZLB_09%> && $("#diaOrderType").val()!="总成订单") {
        ...
        // 总成订单的情况
        if ($("#diaOrderType").val()=="总成订单") {
            if ($obj.val()>1) {
                alertMsg.warn("只允许订购一个总成件！");
                return false;
            }
        }
    ...
    function doCheckbox(checkbox) {
        ...
        arr.push($tr.attr("BELONG_ASSEMBLY"));
