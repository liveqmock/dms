...
$(function(){
	$("#orderId").val(orderId);
	var url = diaSaveAction +"/oldPartByOrderId.ajax?orderId="+orderId;
	...
	switch(parseInt($tabs.attr("currentIndex")))
 	{
		case 0:
			if($true){
		    	...
		    	var url =diaSaveAction+"/returnPartSearch.ajax?orderId="+orderId;
...
function searchCallBack(res){
	try
	{
		...
		if(rows && rows.length > 0)
		{
			...
			setEditValue("dia-oldpartTab",objxml);
		}else
		{
			checkVinCallBack(2);
				...
				<table style="display:none;width:100%;" id="dia_oldPartList" name="dia_oldPartList" ref="dia_oldPart" refQuery="di_oldPartTab">
					<thead>
						<tr>
							...
							<th fieldname="ACTUL_COUNT">缺失数量</th>
