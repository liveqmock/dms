...
import com.org.dms.dao.part.storageMng.stockOutMng.SaleStockOutMngDao;
 ...
 * @date : 2014-07-23
 */
public class SaleStockOutMngAction {
    //日志类
    ...
    //定义dao对象
    private SaleStockOutMngDao dao = SaleStockOutMngDao.getInstance(atx);
    ...
    public void searchSale() throws Exception {
        ...
        try {
            ...
            String WAREHOUSE_TYPE = Pub.val(request, "WAREHOUSE_TYPE");
            //BaseResultSet：结果集封装对象
            BaseResultSet bs = dao.searchSale(page, user, conditions, WAREHOUSE_ID, WAREHOUSE_TYPE);
            //输出结果集，第一个参数”bs”为固定名称，不可修改
            atx.setOutData("bs", bs);
        } catch (Exception e) {
            logger.error(e);
    ...
    public void searchOutBillPart() throws Exception {
        ...
        try {
            ...
            BaseResultSet bs = dao.searchOutBillPart(page, user, conditions, ISSUE_ID);
            //输出结果集，第一个参数”bs”为固定名称，不可修改
            atx.setOutData("bs", bs);
        } catch (Exception e) {
            logger.error(e);
    ...
    public void partStockOutInEdit() throws Exception {
        ...
        try {
            ...
            if(checkUnique.getRowCount()>0){
                OUT_ID = checkUnique.getString(1,"OUT_ID");
            }else{
                /**生成出库单主表信息**/
...
//                d_vo.setCreateUser(user.getAccount());
//                d_vo.setCreateTime(Pub.getCurrentDate());
//                // 新增
//                dao.insertOutBillDtl(d_vo,user);
            	if ("myNull".equals(REMARK[i])) {
                    REMARK[i] = "";
                }
                //校验出库单明细是否存在
...
//                    c_vo.setOutCountTmp(String.valueOf(Integer.parseInt(OUT_AMOUNT[i])-Integer.parseInt(getOld.getString(1, "OUT_COUNT"))));
            ...
            if ("true".equals(flag)) {
	            ...
	            i_vo.setUpdateUser(user.getAccount());
...
//	            dao.insertOutFlow(OUT_ID, account);
	            ...
	            String url="销售出库。/action/part/storageMng/stockInMng/stockOutMng/SaleStockOutMngAction/partStockOutInEdit.ajax";
	            ...
	            if(chkout.getRowCount()==0){
	                ...
	                outVO.setUpdateUser(user.getAccount());
...
//	                }
	...
	//                /******************************************************************/
	//                /**
	//                 * 针对军品订单在出库时转化为实销单，实销单客户为军品销售订单中所填写客户名称
	//                 */
	//                QuerySet getType = dao.getOrderType(orderId);
	//                String type = getType.getString(1, 1);
	//                if(DicConstant.DDLX_08.equals(type)){
     ...
     * @throws Exception
     */
    public void createDealerStockIn(String OrderId,String issueId) throws Exception{
        User user = (User) atx.getSession().get(Globals.USER_KEY);
        ...
        createDealerStockOut(OrderId,issueId);
    }
     ...
     * @throws Exception
     */
    public void createDealerStockOut(String OrderId,String issueId) throws Exception{
        User user = (User) atx.getSession().get(Globals.USER_KEY);
    ...
    public void partStockOutInIndex() throws Exception {
        ...
        try {
            ...
            QuerySet getType = dao.getOrderType(orderId);
            String type = getType.getString(1, 1);
            if(type.equals(DicConstant.DDLX_08)){
                this.createRealSaleOrder(orderId);
            }
            atx.setOutMsg("", "出库成功！");
        } catch (Exception e) {
            atx.setException(e);
...
//        dao.insertOutFlow(outId, user);
    ...
    public void createRealSaleOrder(String OrderId) throws Exception{
        User user = (User) atx.getSession().get(Globals.USER_KEY);
        QuerySet getSaleInfo = dao.getSaleInfo(OrderId);//获取军品销售订单主表信息
        String CustomerName = getSaleInfo.getString(1,"CUSTORM_NAME");
        ...
        String ORG_ID = getSaleInfo.getString(1, "ORG_ID");
        QuerySet getSaleDtlInfo = dao.getSaleDtlInfo(OrderId);//获取军品销售单明细表信息
        ...
        rsVo.setSaleId(SequenceUtil.getCommonSerivalNumber(atx.getDBFactory()));
        ...
        for(int i= 1;i<=getSaleDtlInfo.getRowCount();i++){
            ...
            rsDtlVO.setDtlId(SequenceUtil.getCommonSerivalNumber(atx.getDBFactory()));
...
//            String org_id = user.getOrgId();
            ...
            QuerySet checkArmy = dao.getIfArmy(ACCOUNT);
            String warehouse = "";
            if(checkArmy.getRowCount()>0){
                for(int i=0;i<checkArmy.getRowCount();i++){
                    String typeString = checkArmy.getString(1, "WAREHOUSE_TYPE");
                    if(typeString.equals("100102")){
                        warehouse = checkArmy.getString(1, "WAREHOUSE_ID");
                    }else if(typeString.equals("100105")){
                    	warehouse = checkArmy.getString(1, "WAREHOUSE_ID");
                    }else{
                        String type=  "100101";
                        QuerySet getWid = dao.getWare(type);
                        warehouse = getWid.getString(1, 1);
                    }
                }
            }else{
                warehouse = null;
            ...
            //BaseResultSet：结果集封装对象
            BaseResultSet bs = dao.searchSaleOrder(page, user, conditions,warehouse,ACCOUNT,partCode,partName);
            //输出结果集，第一个参数”bs”为固定名称，不可修改
            atx.setOutData("bs", bs);
        } catch (Exception e) {
            logger.error(e);
