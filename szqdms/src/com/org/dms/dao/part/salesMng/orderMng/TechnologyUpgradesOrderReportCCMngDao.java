 ...
 * @version 1.0
 */
public class TechnologyUpgradesOrderReportCCMngDao extends BaseDAO {
     ...
     */
    public static final TechnologyUpgradesOrderReportCCMngDao getInstance(ActionContext pActionContext) {

        TechnologyUpgradesOrderReportCCMngDao technologyUpgradesOrderReportMngDao = new TechnologyUpgradesOrderReportCCMngDao();
    ...
    public BaseResultSet searchPart(PageManager pageManager, User pUser, String pConditions, String pOrderId,String warehouseId) throws Exception {
        ...
        sql.append("       T.STOCK,\n" );
        sql.append("       T.PLAN_PRICE SALE_PRICE\n" );
...
//        sql.append("                   AND A.SALE_PRICE > 0) D LEFT JOIN\n" );
    ...
    public BaseResultSet dealerSearchDirectBusinessOrder(PageManager page,String pConditions, User pUser) throws Exception {
        ...
        String wheres = pConditions;
        wheres += " (A.ORDER_ID =B.DIR_SOURCE_ORDER_ID OR A.ORDER_ID=B.ORDER_ID) AND A.STATUS = '" + DicConstant.YXBS_01 + "'\n";
        page.setFilter(wheres);
        BaseResultSet baseResultSet = null;
        StringBuilder sql= new StringBuilder();
     ...
     * @throws Exception
     */
    public QuerySet searchOrgId(String orgId) throws Exception {

        StringBuffer sql= new StringBuffer();
        sql.append("SELECT ORG_ID,CODE,ONAME from tm_org WHERE ORG_ID =(SELECT DC_ID FROM PT_BA_SERVICE_DC WHERE ORG_ID='" + orgId + "' AND IF_DEFAULT='" + DicConstant.SF_01 + "') \n" );
        return factory.select(null, sql.toString());
    }

    public void updateOrderDtl(String orderId)throws Exception {
    	String sql1 = "DELETE FROM PT_BU_SALE_ORDER_DTL WHERE ORDER_ID = (SELECT DIR_SOURCE_ORDER_ID FROM PT_BU_SALE_ORDER WHERE ORDER_ID="+orderId+")\n";
    	...
    	sql.append("   AMOUNT)\n" );
    	sql.append("  SELECT F_GETID(),(SELECT DIR_SOURCE_ORDER_ID FROM PT_BU_SALE_ORDER WHERE ORDER_ID="+orderId+"),\n" );
    	sql.append("         PART_ID,\n" );
    	sql.append("         PART_CODE,\n" );
    	sql.append("         PART_NAME,\n" );
    	sql.append("         ORDER_COUNT,\n" );
