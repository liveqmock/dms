 ...
 * @date : 2014-08-11
 */
public class GuaranteeStockOutMngDao extends BaseDAO {
    //定义instance
    public static final GuaranteeStockOutMngDao getInstance(ActionContext atx) {
        GuaranteeStockOutMngDao dao = new GuaranteeStockOutMngDao();
    ...
    public BaseResultSet searchOutBill(PageManager page, User user, String conditions,String account) throws Exception {
        ...
        wheres.append(" AND A.ORDER_TYPE ='" + DicConstant.DDLX_09 + "'\n" );
        wheres.append(" AND A.ORDER_ID = B.ORDER_ID\n" );
        wheres.append(" AND B.CHECK_USER = C.ACCOUNT\n" );
        wheres.append(" AND B.ORDER_ID = D.ORDER_ID\n" );
        wheres.append(" AND D.ISSUE_STATUS = '" + DicConstant.FLDFLZT_01 + "'\n" );
        wheres.append(" AND D.USER_ACCOUNT = '" + account + "'\n" );
        wheres.append(" AND D.PRINT_STATUS = '" + DicConstant.DYZT_02 + "'\n" );
        wheres.append(" AND A.WAREHOUSE_ID IN (SELECT WAREHOUSE_ID FROM PT_BA_WAREHOUSE WHERE WAREHOUSE_TYPE=100101)\n" );
        wheres.append(" AND A.SHIP_STATUS = '" + DicConstant.DDFYZT_02 + "'\n" );
        wheres.append(" AND A.ORDER_STATUS = '" + DicConstant.DDZT_03 + "'\n" );
//        wheres.append(" AND NOT EXISTS(SELECT 1 FROM PT_BU_STOCK_OUT D WHERE D.ORDER_ID = A.ORDER_ID)\n" );
        wheres.append(" AND A.STATUS = '" + DicConstant.YXBS_01 + "'\n" );
        wheres.append(" AND A.OEM_COMPANY_ID='" + user.getOemCompanyId() + "'\n" );
        wheres.append(" ORDER BY A.APPLY_DATE DESC");
     ...
     * @throws Exception
     */
    public BaseResultSet searchSale(PageManager page, User user, String conditions, String WAREHOUSE_ID) throws Exception {
        String wheres = conditions;

        wheres += " AND A.ORDER_TYPE = " + DicConstant.DDLX_09 + "\n";
...
//     * @throws Exception
//     */
//    public BaseResultSet searchOutBillPart(PageManager page, User user, String conditions, String OUT_ID, String ORDER_ID) throws Exception {
//        StringBuilder wheres = new StringBuilder(conditions);
//        wheres.append("   AND A.ORDER_ID = " + ORDER_ID + "\n");
...
//        wheres.append("   AND A.ISSUE_ID = E.ISSUE_ID\n");
...
//        sql.append("       NVL(B.OUT_AMOUNT,0) OUT_AMOUNT,\n");
//        sql.append("       D.AMOUNT,\n");
...
//        sql.append("       (SELECT * FROM PT_BU_STOCK_OUT_DTL WHERE OUT_ID = " + OUT_ID + ") B,\n");
...
//    }
     ...
     * @throws Exception
     */
    public BaseResultSet searchOutBillPart(PageManager page, User user, String conditions, String ISSUE_ID,String account) throws Exception {
        //定义返回结果集
        BaseResultSet bs = null;
...
//        sql.append("   ORDER BY A.PART_CODE ASC");
    ...
    public QuerySet getWareInfo(String orderId) throws Exception
    {
    	...
    	sql.append("AND T3.WAREHOUSE_TYPE IN(100101,100102)\n" );
    ...
    public QuerySet checkOut(String orderId)throws Exception
    {
    	QuerySet qs = null;
    	StringBuffer sql= new StringBuffer();
    	sql.append("SELECT OUT_ID FROM PT_BU_STOCK_OUT WHERE ORDER_ID = "+orderId+"");
    	qs = factory.select(null, sql.toString());
    	return qs;
    }
...
//        sql.append("           AND B.POSITION_ID = A.POSITION_ID)\n");
     ...
     * @throws Exception
     */
    public boolean updateIssueOrderDtl(String ORDER_ID, String OUT_ID, User user)
            throws Exception {
        StringBuilder sql = new StringBuilder();
        sql.append("UPDATE PT_BU_ISSUE_ORDER_DTL A\n");
        sql.append("   SET A.REAL_COUNT =\n");
        ...
        sql.append("       A.UPDATE_USER = '" + user.getAccount() + "'\n");
        sql.append(" WHERE 1 = 1\n");
        sql.append("   AND A.ORDER_ID = " + ORDER_ID + "\n");
        sql.append("   AND EXISTS (SELECT 1\n");
        sql.append("          FROM PT_BU_STOCK_OUT_DTL B\n");
        sql.append("         WHERE B.OUT_ID = " + OUT_ID + "\n");
        sql.append("           AND B.PART_ID = A.PART_ID\n");
