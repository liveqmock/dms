 ...
 * @date : 2014-07-23
 */
public class SaleStockOutMngDao extends BaseDAO {
    //定义instance
    public static final SaleStockOutMngDao getInstance(ActionContext atx) {
        SaleStockOutMngDao dao = new SaleStockOutMngDao();
    ...
    public BaseResultSet searchOutBill(PageManager page, User user, String conditions) throws Exception {
        ...
        wheres.append("   AND A.OUT_TYPE = " + DicConstant.CKLX_01 + "\n");
     ...
     * @param user
     * @param conditions
     * @param WAREHOUSE_ID   出库仓库ID
     * @param WAREHOUSE_TYPE 出库仓库类型
     * @return
     * @throws Exception
     */
    public BaseResultSet searchSale(PageManager page, User user, String conditions, String WAREHOUSE_ID, String WAREHOUSE_TYPE) throws Exception {
        String wheres = conditions;
        if ("100101".equals(WAREHOUSE_TYPE)) {//民品库
            wheres += "   AND A.ORDER_TYPE IN(" + DicConstant.DDLX_01 + "," + DicConstant.DDLX_02 + "," + DicConstant.DDLX_03 + "," + DicConstant.DDLX_04 + "," + DicConstant.DDLX_06 + "," + DicConstant.DDLX_09 + ","+DicConstant.DDLX_11+","+DicConstant.DDLX_12+")\n";
        } else {//军品库
            wheres += "   AND A.ORDER_TYPE = " + DicConstant.DDLX_08 + "\n";
        }
        ...
        sql.append("  FROM PT_BU_SALE_ORDER A, (SELECT ORDER_ID, CHECK_USER, MAX(CHECK_DATE) CHECK_DATE\n");
        sql.append("  FROM PT_BU_SALE_ORDER_CHECK\n");
        sql.append(" WHERE CHECK_RESULT = 202203\n");
        sql.append(" GROUP BY ORDER_ID, CHECK_USER) B, B, TM_USER C\n");
     ...
     * @throws Exception
     */
    public BaseResultSet searchOutBillPart(PageManager page, User user, String conditions, String ISSUE_ID) throws Exception {
        //定义返回结果集
        BaseResultSet bs = null;
...
//        sql.append("                     FROM PT_BU_ISSUE_ORDER_DTL B,PT_BU_ISSUE C\n");
...
//        sql.append("           AND B.PART_ID = A.PART_ID)\n");
     ...
     * @throws Exception
     */
    public boolean updateStockDtl(String ORDER_ID, String account)
            throws Exception {
        StringBuilder sql = new StringBuilder();
...
//        sql.append("                      AND B.PART_ID = A.PART_ID\n");
//        sql.append("                      AND B.SUPPLIER_ID = A.SUPPLIER_ID\n");
//        sql.append("                      AND B.ISSUE_ID = A.ISSUE_ID\n");
//        sql.append("                      AND C.USER_ACCOUNT = '"+account+"'\n");
//        sql.append("                      AND B.POSITION_ID = A.POSITION_ID),\n");
//        sql.append("       A.UPDATE_TIME = SYSDATE,\n");
//        sql.append("       A.UPDATE_USER = '" + account + "'\n");
...
//        StringBuffer sql= new StringBuffer();
        ...
        sql.append("                              AND B.POSITION_ID = A.POSITION_ID),\n" );
        sql.append("               A.OCCUPY_AMOUNT = A.OCCUPY_AMOUNT -\n" );
        sql.append("                          (SELECT NVL(B.SHOULD_COUNT,0)\n" );
        sql.append("                             FROM PT_BU_ISSUE_ORDER_DTL B,PT_BU_ISSUE_ORDER C\n" );
        sql.append("                            WHERE B.ORDER_ID = "+ORDER_ID+"\n" );
        sql.append("                              AND B.PART_ID = A.PART_ID\n" );
        sql.append("                              AND B.ISSUE_ID = C.ISSUE_ID\n" );
    ...
    public boolean updateIssueOrder(String ORDER_ID, User user)
        ...
        sql.append("   SET A.ISSUE_STATUS = " + DicConstant.FLDFLZT_02 + ", A.UPDATE_USER = '" + user.getAccount() + "', A.UPDATE_TIME = SYSDATE\n");
    ...
    public boolean insertOutFlow(String OUT_ID, String account) throws Exception {
        ...
        sql.append("   WHERE A.OUT_ID = B.OUT_ID AND C.ORDER_ID = B.ORDER_ID AND C.USER_ACCOUNT = '"+account+"' AND A.PART_ID = D.PART_ID AND C.ISSUE_ID = D.ISSUE_ID\n");
     ...
     * @param ORDER_ID
     */
    public void updateOutBillDtl(String OUT_ID, String ORDER_ID) throws Exception {
        StringBuilder sql = new StringBuilder();
    ...
    public QuerySet getOrderType(String orderId) throws Exception
    {
        QuerySet qs = null;
        StringBuffer sql= new StringBuffer();
        sql.append("SELECT ORDER_TYPE\n" );
        sql.append("  FROM PT_BU_SALE_ORDER WHERE ORDER_ID = "+orderId+"\n" );
        qs = factory.select(null, sql.toString());
        return qs;
    }
    
    public boolean inserRealSale(PtBuRealSaleVO vo)
            throws Exception {
        return factory.update(vo);
    }
    public boolean insertRealSaleDtl(PtBuRealSaleDtlVO vo)
            throws Exception {
        return factory.update(vo);
    }
...
//        QuerySet qs = null;
//        StringBuffer sql= new StringBuffer();
//        sql.append("SELECT A.ORG_ID,A.CUSTORM_NAME, A.DELIVERY_ADDR, A.PHONE, NVL(B.AMOUNT,0)AMOUNT, NVL(B.COUNT,0)COUNT\n" );
//        sql.append("  FROM PT_BU_SALE_ORDER A,\n" );
//        sql.append("       (SELECT SUM(T.AUDIT_COUNT * T.UNIT_PRICE) AMOUNT,\n" );
//        sql.append("               SUM(T.AUDIT_COUNT) COUNT,\n" );
...
//        sql.append("  FROM PT_BU_ISSUE_ORDER_DTL T, PT_BU_SALE_ORDER_DTL T1\n" );
//        sql.append(" WHERE T.PART_ID = T1.PART_ID  AND T.ORDER_ID = T1.ORDER_ID");
//        sql.append("   AND T.ORDER_ID = "+orderId+"");
//        qs = factory.select(null, sql.toString());
//        return qs;
//    }
    ...
    public QuerySet getSaleDtlInfo(String orderId) throws Exception {
        ...
        sql.append("       T.SUPPLIER_NAME,\n");
        sql.append("       T1.UNIT_PRICE SALE_PRICE,\n");
        sql.append("       SUM(T.REAL_COUNT) SHOULD_COUNT,\n");
...
//    public BaseResultSet searchSaleOrder(PageManager page, User user, String conditions,String warehouse,String ACCOUNT,String partCode,String partName) throws Exception {
//
//        String wheres = conditions+" AND D.USER_ACCOUNT='"+user.getAccount()+"' AND ISSUE_STATUS = '"+DicConstant.FLDFLZT_01+"'\n";
//
//        
//        
//        
//        if(!"".equals(partCode)&&partCode!=null){
//            wheres +="AND A.ORDER_TYPE IN\n" +
//                    "       (203701, 203702, 203703, 203704, 203706, 203708, 203710)\n" + 
//                    "   AND A.ORDER_ID = B.ORDER_ID\n" + 
//                    "   AND B.CHECK_USER = C.ACCOUNT\n" + 
//                    "   AND B.ORDER_ID = D.ORDER_ID\n" + 
//                    "   AND D.USER_ACCOUNT = '"+ACCOUNT+"'\n" + 
//                    "   AND D.PRINT_STATUS = "+DicConstant.DYZT_02+"\n" + 
//                    "   AND A.WAREHOUSE_ID = "+warehouse+"\n" + 
//                    "   AND A.SHIP_STATUS = "+DicConstant.DDFYZT_02+"\n" + 
//                    "   AND A.ORDER_STATUS = "+DicConstant.DDZT_03+"\n" + 
//                    "   AND B.CHECK_RESULT = "+DicConstant.DDZT_03+"\n" + 
////                    "   AND NOT EXISTS\n" + 
//                    //" (SELECT 1 FROM PT_BU_ISSUE_ORDER D WHERE D.ORDER_ID = A.ORDER_ID AND D.ISSUE_STATUS = "+DicConstant.FLDFLZT_01+" AND D.USER_ACCOUNT = "+user.getAccount()+")\n" + 
//                    "   AND A.STATUS = 100201\n" + 
//                    "    AND EXISTS (SELECT 1 FROM PT_BU_SALE_ORDER_DTL E WHERE E.ORDER_ID = A.ORDER_ID AND E.PART_CODE LIKE '"+partCode+"')\n"+
...
//                    "   AND B.CHECK_RESULT = "+DicConstant.DDZT_03+"\n" + 
////                    "   AND NOT EXISTS\n" + 
////                    " (SELECT 1 FROM PT_BU_STOCK_OUT D WHERE D.ORDER_ID = A.ORDER_ID)\n" + 
//                    "    AND EXISTS (SELECT 1 FROM PT_BU_SALE_ORDER_DTL E WHERE E.ORDER_ID = A.ORDER_ID AND E.PART_NAME LIKE '"+partName+"')\n"+
//                    "   AND A.STATUS = 100201\n" + 
//                    " ORDER BY A.APPLY_DATE DESC";
...
////                    " (SELECT 1 FROM PT_BU_STOCK_OUT D WHERE D.ORDER_ID = A.ORDER_ID)\n" + 
//                    "   AND A.STATUS = 100201\n" + 
//                    "    AND EXISTS (SELECT 1 FROM PT_BU_SALE_ORDER_DTL E WHERE E.ORDER_ID = A.ORDER_ID AND E.PART_NAME LIKE '"+partName+"' AND E.PART_CODE LIKE '"+partCode+"')\n"+
//                    " ORDER BY A.APPLY_DATE DESC";
//            }else{
//            wheres +="AND A.ORDER_TYPE IN\n" +
//                    "       (203701, 203702, 203703, 203704, 203706, 203708, 203710)\n" + 
...
//        sql.append("                 2),\n" );
//        sql.append("           0) RATE1");
//        sql.append("  FROM PT_BU_SALE_ORDER A, PT_BU_SALE_ORDER_CHECK B, TM_USER C,PT_BU_ISSUE_ORDER D\n" );
///*        sql.append(" WHERE  1=1    AND A.ORDER_TYPE IN(203701,203702,203703,203704,203706,203708,203710)\n" );
...
//        sql.append(" AND A.WAREHOUSE_ID = "+warehouse+"\n" );
...
//    }
    ...
    public BaseResultSet searchSaleOrder(PageManager page, User user, String conditions,String warehouse,String ACCOUNT,String partCode,String partName) throws Exception {
        ...
        if(!"".equals(partCode)&&partCode!=null){
            wheres +="AND A.ORDER_TYPE IN\n" +
                    "       (203701, 203702, 203703, 203704, 203706, 203708, 203710,203711,203712)\n" + 
                    "   AND A.ORDER_ID = B.ORDER_ID\n" + 
                    "   AND B.CHECK_USER = C.ACCOUNT\n" + 
                    "   AND B.ORDER_ID = D.ORDER_ID\n" + 
                    "   AND D.USER_ACCOUNT = '"+ACCOUNT+"'\n" + 
...
//                    " (SELECT 1 FROM PT_BU_STOCK_OUT D WHERE D.ORDER_ID = A.ORDER_ID)\n" + 
        ...
        sql.append("                 2),\n" );
        sql.append("           0) RATE1");
        sql.append("  FROM PT_BU_SALE_ORDER A, (SELECT ORDER_ID, CHECK_USER, MAX(CHECK_DATE) CHECK_DATE FROM PT_BU_SALE_ORDER_CHECK WHERE CHECK_RESULT = "+DicConstant.DDZT_03+" GROUP BY ORDER_ID, CHECK_USER) B, TM_USER C,PT_BU_ISSUE_ORDER D\n" );
/*        sql.append(" WHERE  1=1    AND A.ORDER_TYPE IN(203701,203702,203703,203704,203706,203708,203710)\n" );
        sql.append(" AND A.ORDER_ID = B.ORDER_ID\n" );
        sql.append(" AND B.CHECK_USER = C.ACCOUNT\n" );
        sql.append(" AND B.ORDER_ID = D.ORDER_ID\n" );
...
*/
    ...
    public QuerySet getIfArmy(String ACCOUNT)throws Exception
    {
        QuerySet qs = null;
        StringBuffer sql= new StringBuffer();
        sql.append("SELECT T.WAREHOUSE_ID,T.WAREHOUSE_TYPE FROM PT_BA_WAREHOUSE T,TM_USER T1 WHERE T.ORG_ID =T1.ORG_ID AND T1.ACCOUNT = '"+ACCOUNT+"'");
        qs = factory.select(null, sql.toString());
        return qs;
    }
    public QuerySet getWare(String type)throws Exception
    {
        QuerySet qs = null;
        StringBuffer sql= new StringBuffer();
        sql.append("SELECT T.WAREHOUSE_ID FROM PT_BA_WAREHOUSE T WHERE T.WAREHOUSE_TYPE = "+type+"");
        qs = factory.select(null, sql.toString());
        return qs;
    }
    public QuerySet getWareInfo(String orderId) throws Exception
    {
        ...
        sql.append("AND T3.WAREHOUSE_TYPE IN(100101,100102,100105)\n" );
        sql.append("AND T.ORDER_ID ="+orderId+"");
        qs = factory.select(null, sql.toString());
        return qs;
    }
    public QuerySet checkOut(String orderId)throws Exception
    {
        QuerySet qs = null;
        StringBuffer sql= new StringBuffer();
        sql.append("SELECT OUT_ID,OUT_NO FROM PT_BU_STOCK_OUT WHERE ORDER_ID = "+orderId+"");
        qs = factory.select(null, sql.toString());
        return qs;
    }
    ...
    public QuerySet getIssue(String PART_ID,String ISSUE_ID) throws Exception
    {
        QuerySet qs = null;
        StringBuffer sql= new StringBuffer();
        sql.append("SELECT T.PART_ID,\n" );
    ...
    public boolean updateIssue(PtBuIssueOrderVO vo)
            throws Exception {
        return factory.update(vo);
    }
    ...
    public QuerySet checkIfAllOut(String orderId,String issueId)throws Exception
    {
        QuerySet qs = null;
        StringBuffer sql= new StringBuffer();
        sql.append("SELECT  1 from PT_BU_ISSUE_ORDER WHERE ORDER_ID = "+orderId+"AND ISSUE_STATUS = "+DicConstant.FLDFLZT_01+" AND ISSUE_ID != "+issueId+"");
        qs = factory.select(null, sql.toString());
    return qs;
    }
