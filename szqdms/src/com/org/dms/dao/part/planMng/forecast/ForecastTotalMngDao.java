 ...
 * @version 1.0
 */
public class ForecastTotalMngDao extends BaseDAO {
     ...
     */
    public static final ForecastTotalMngDao getInstance(ActionContext pActionContext) {

        ForecastTotalMngDao forecastTotalMngDao = new ForecastTotalMngDao();
    ...
    public BaseResultSet searchByForecastMonth1(PageManager page,String conditions,String forecastMonth, User pUser) throws Exception {
    	BaseResultSet baseResultSet=null;
        StringBuffer sql = new StringBuffer();
        String wheres = conditions;
        wheres+=" AND FORCAST_MONTH='" + forecastMonth + "' AND ORG_ID = " + pUser.getOrgId() + "\n";
    ...
    public BaseResultSet searchForecastByDealer(PageManager pPageManager, User pUser, String pConditions) throws Exception {
        ...
        wheres += " AND PBF.ORG_ID=TMO.ORG_ID \n"
                + " AND PBF.FORCAST_STATUS=" + DicConstant.PJYCZT_02 + " AND TMO.ORG_TYPE IN ('"+DicConstant.ZZLB_10+"','"+DicConstant.ZZLB_11+"')\n"
                + " AND PBF.FORCAST_ORG_ID IN( SELECT ORG_ID FROM PT_BA_SERVICE_DC WHERE DC_ID = "+pUser.getOrgId()+" AND IF_DEFAULT = "+DicConstant.SF_01+")\n"
    ...
    public boolean insertForecastDetail(User pUser, String pConditions) throws Exception {
        ...
        sql.append("        WHERE FORCAST_ID IN \n");
        sql.append("            (SELECT FORCAST_ID \n");
        sql.append("            FROM PT_BU_FORCAST PBF \n");
        sql.append("            WHERE PBF.FORCAST_MONTH = '" + pConditions+ "' \n");
        sql.append("            AND PBF.ORG_ID IN \n");
        sql.append("                (SELECT ORG_ID \n");
        sql.append("                FROM PT_BA_SERVICE_DC \n");
        sql.append("                WHERE DC_ID = '" + pUser.getOrgId()+ "' \n");
        sql.append("                AND IF_DEFAULT = " + DicConstant.SF_01 + "\n");
        sql.append("                )\n");
        sql.append("            ) \n");
