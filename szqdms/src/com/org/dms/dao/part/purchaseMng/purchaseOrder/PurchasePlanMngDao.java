...
public class PurchasePlanMngDao extends BaseDAO{
	 public  static final PurchasePlanMngDao getInstance(ActionContext atx)
	    {
		 PurchasePlanMngDao dao = new PurchasePlanMngDao();
	 ...
	 public BaseResultSet partSearch(PageManager page, User user, String conditions,String type,String account) throws Exception
	    {
	    	...
	    	wheres += " AND T.PART_ID = T1.PART_ID AND T1.STATUS="+DicConstant.YXBS_01+" AND T.ORDER_COUNT>0  AND T1.USER_ACCOUNT = '"+account+"'\n";
	    	...
	    	if(type.equals(DicConstant.CGDDLX_02)){
	    		sql.append("DECODE(1,\n" );
		    	sql.append("       (SELECT DISTINCT 1\n" );
		    	sql.append("          FROM PT_BU_SALE_ORDER_DTL X\n" );
		    	sql.append("         WHERE X.PART_ID = T.PART_ID\n" );
		    	sql.append("           AND EXISTS (SELECT 1\n" );
		    	sql.append("                  FROM PT_BU_SALE_ORDER X1\n" );
		    	sql.append("                 WHERE X.ORDER_ID = X1.ORDER_ID\n" );
		    	sql.append("                   AND X1.IF_DELAY_ORDER = 100101\n" );
		    	sql.append("                   AND X1.IF_CHANEL_ORDER = 100102\n" );
		    	sql.append("                   AND X1.ORDER_STATUS = 202202\n" );
		    	sql.append("                   AND X1.ORDER_TYPE = 203709)),\n" );
		    	sql.append("       '是',\n" );
		    	sql.append("       '否') AS IF_SB,");
	    	}else{
	    		sql.append("   '否' AS IF_SB,");
	    	}
	    	sql.append("       T.UNIT,\n" );
	    	...
	    	sql.append("       T1.USER_ACCOUNT,\n" );
	    	sql.append("       (SELECT TO_CHAR(WM_CONCAT(I.AREA_CODE))\n" );
	    	...
	    	sql.append("                                    AND J.WAREHOUSE_TYPE = 100101\n" );
	    	...
	    	sql.append("               T.SUPPLIER_NAME,\n" );
	    	sql.append("               T.PLAN_PRICE,\n" );
	    	sql.append("               (NVL(T.ORDER_COUNT,0) + NVL(S.LOWER_LIMIT,0)- T.PCH_COUNT - NVL(S.AVAILABLE_AMOUNT,0)) ORDER_COUNT,\n" );
	    	sql.append("               (NVL(T.ORDER_COUNT,0) + NVL(S.LOWER_LIMIT,0)- T.PCH_COUNT - NVL(S.AVAILABLE_AMOUNT,0)) * T.PLAN_PRICE ALL_PRICE,\n" );
	    	...
	    	sql.append("                           AND B.PART_ID = E.PART_ID AND D.PART_IDENTIFY = "+DicConstant.YXBS_01+"");
	    	sql.append("                           AND C.PART_STATUS = "+DicConstant.PJZT_01+" AND E.PART_IDENTIFY = "+DicConstant.YXBS_01+"\n" );
	    	sql.append("                           AND B.IF_SUPPLIER = "+DicConstant.SF_01+"\n" );
	    	sql.append("                           AND C.IF_DIRECT = "+DicConstant.SF_02+"\n" );
	    	sql.append("                           AND A.IF_DELAY_ORDER = "+DicConstant.SF_01+"\n" );
	    	sql.append("                           AND A.ORDER_TYPE NOT IN ("+DicConstant.DDLX_05+", "+DicConstant.DDLX_08+")\n" );
	    	sql.append("                           AND A.IF_CHANEL_ORDER = "+DicConstant.SF_02+"\n" );
	    	if(!DicConstant.CGDDLX_06.equals(type)){
	    	...
	    	if(type.equals(DicConstant.CGDDLX_01)){
				sql.append(" AND A.ORDER_TYPE = "+DicConstant.DDLX_01+"\n");
			}else if(type.equals(DicConstant.CGDDLX_06)){
				sql.append(" AND A.ORDER_TYPE = "+DicConstant.DDLX_04+"\n");
			}else if(type.equals(DicConstant.CGDDLX_07)){
				sql.append(" AND A.ORDER_TYPE = "+DicConstant.DDLX_10+"\n");
			}else{
				sql.append(" AND A.ORDER_TYPE NOT IN ("+DicConstant.DDLX_01+","+DicConstant.DDLX_04+","+DicConstant.DDLX_10+")\n");
			}
	    	...
	    	sql.append("                               AND B.PART_ID = C.PART_ID\n" );
	    	//if(DicConstant.CGDDLX_02.equals(type)){
	    		sql.append("                           AND A.ORDER_TYPE IN ("+type+","+DicConstant.CGDDLX_03+")\n" );
	    	//}else{
	    	//	sql.append("                           AND A.ORDER_TYPE = "+type+"\n" );
	    	//}
	    	...
	    	sql.append("                       FROM PT_BU_STOCK E WHERE E.WAREHOUSE_ID = (SELECT WAREHOUSE_ID FROM PT_BA_WAREHOUSE WHERE WAREHOUSE_TYPE = 100101 AND WAREHOUSE_STATUS = "+DicConstant.YXBS_01+"))T1\n" );
	    	...
	    	sql.append("               T.SUPPLIER_NAME,\n" );
	    	sql.append("               T.PLAN_PRICE,\n" );
	    	sql.append("               (NVL(T.ORDER_COUNT,0)+NVL(S.LOWER_LIMIT,0)-NVL(PCH_COUNT,0) - NVL(S.AVAILABLE_AMOUNT,0)) ORDER_COUNT,\n" );
	    	sql.append("               (NVL(T.ORDER_COUNT,0)+NVL(S.LOWER_LIMIT,0)-NVL(PCH_COUNT,0) - NVL(S.AVAILABLE_AMOUNT,0)) * T.PLAN_PRICE ALL_PRICE,\n" );
	    	sql.append("               S.AVAILABLE_AMOUNT,\n" );
	    	sql.append("               S.UPPER_LIMIT,\n" );
	    	sql.append("               S.LOWER_LIMIT\n" );
	    	sql.append("          FROM (SELECT T1.PART_ID,\n" );
	    	...
	    	sql.append("						   AND A.ORDER_STATUS = "+DicConstant.DDZT_02+"\n" );
	    	sql.append("                           AND C.IF_DIRECT = "+DicConstant.SF_02+"\n" );
	    	sql.append("                           AND D.SUPPLIER_ID = E.SUPPLIER_ID AND D.PART_IDENTIFY = "+DicConstant.YXBS_01+"\n" );
	    	sql.append("                           AND C.PART_STATUS = "+DicConstant.PJZT_01+" AND E.PART_IDENTIFY = "+DicConstant.YXBS_01+"\n" );
...
//	    	}
	    	...
	    	sql.append("                        AND T1.SUPPLIER_ID = F.SUPPLIER_ID\n" );
	    	sql.append("                        AND T1.WAREHOUSE_ID = F.STOCK_ID) S\n" );
	    	sql.append("            ON T.PART_ID = S.PART_ID) T,PT_BA_PCH_ATTRIBUTE T1\n" );
	 ...
	 public BaseResultSet partReSearch(PageManager page, String account, String conditions,String type) throws Exception
	    {
	    	...
	    	BaseResultSet bs = null;
	    	StringBuffer sql= new StringBuffer();
//	    	sql.append("SELECT T1.PART_ID||T1.SUPPLIER_ID KEY_ID,T1.PART_ID,T1.PART_CODE,T1.PART_NAME,T1.PART_NO,T1.UNIT,T1.MIN_PACK,T1.MIN_UNIT,T1.IF_SUPLY,T1.SUPPLIER_ID,T1.APPLY_CYCLE,T1.SUPPLIER_CODE,T1.SUPPLIER_NAME,T1.ROWSPAN,T1.PLAN_PRICE,T1.ORDER_COUNT,T1.ALL_PRICE,T1.AVAILABLE_AMOUNT,T1.UPPER_LIMIT,T1.LOWER_LIMIT,T1.USER_ACCOUNT,T1.POSITION_NAME,T1.PCH_COUNT,T1.REMARK,T1.PLAN_TYPE FROM PT_BU_PURCHASEPLAY_TMP T1");
	    	sql.append("SELECT T.PART_ID || T.SUPPLIER_ID KEY_ID,\n" );
	    	sql.append("       T.PART_ID,\n" );
	    	sql.append("       T.PART_CODE,\n" );
	    	sql.append("       T.PART_NAME,\n" );
	 ...
	 public QuerySet getPart(String PART_ID,String SUPPLIER_ID) throws Exception
	    {
	    	QuerySet qs = null;
	    	StringBuffer sql= new StringBuffer();
	    	sql.append("SELECT T.APPLY_CYCLE,NVL(T.PCH_PRICE,0) PCH_PRICE,NVL(T1.PLAN_PRICE,0) PLAN_PRICE,T1.PART_CODE,T1.PART_NAME FROM PT_BA_PART_SUPPLIER_RL T,PT_BA_INFO T1 WHERE T.PART_ID = T1.PART_ID AND T.PART_ID = "+PART_ID+" AND T.SUPPLIER_ID="+SUPPLIER_ID+" AND T.PART_IDENTIFY = "+DicConstant.YXBS_01+"");
	    	qs = factory.select(null, sql.toString());
	    	return qs;
	    }
